...
`;
  ...
  function _shell(){return`
    <div class="ph">
      ...
      <div class="phr">
        <button id="bl-logout-btn" class="ibtn red hidden" onclick="App.logout()" title="로그아웃">🚪</button>
      </div>
    </div>
    <div class="bl-stabs">
    ...
    return`<div class="bl-mstats" id="bl-mstats">
      ...
      <div style="display:flex;gap:6px;align-items:center;flex-shrink:0">
        ...
        <button class="bl-report-btn" onclick="BooklibApp.openClassReport()">📋 전체 출력</button>
         ...
         <button class="bl-report-btn" onclick="BooklibApp.openStampSettings()" title="타임스탬프 기준 비율 설정 (xlsx/CSV/확장 프로그램 공통)" style="background:rgba(139,92,246,.1);border-color:rgba(139,92,246,.4);color:#7c3aed">📍 스탬프 기준</button>
    ...
    modal.innerHTML = `
      <div style="background:var(--card);border-radius:20px 20px 0 0;width:100%;max-width:540px;padding:20px 20px 32px;box-shadow:var(--sh2);animation:slideUp .22s ease;max-height:90vh;overflow-y:auto" onclick="event.stopPropagation()">
        ...
        <div style="display:flex;align-items:center;gap:8px;padding:8px 10px;background:var(--surf2);border-radius:8px;border:1px solid var(--bdr2);margin-bottom:12px">
          ...
          <span id="bl-stamp-thresh-csv-val" style="font-size:12px;font-weight:800;color:var(--a);min-width:34px;text-align:right">${Number(localStorage.getItem('bl_stamp_threshold')||50)}%</span>
        </div>
  ...
  function openStampSettings(){
    ...
    const modal = document.createElement('div');
    modal.id = 'bl-stamp-settings';
    modal.style.cssText = 'position:fixed;inset:0;background:rgba(0,0,0,.45);z-index:300;display:flex;align-items:center;justify-content:center';
    modal.onclick = e=>{ if(e.target===modal) modal.remove(); };
