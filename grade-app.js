...
.gcm-hdr-info{flex:1;min-width:0;}.gcm-hdr-title{font-size:16px;font-weight:900;color:var(--tx);}
.gcm-hdr-sub{font-size:12px;color:var(--tx3);margin-top:2px;}
.gcm-hdr-close{width:34px;height:34px;border-radius:50%;border:none;background:var(--surf2);cursor:pointer;font-size:16px;color:var(--tx2);display:flex;align-items:center;justify-content:center;transition:background .12s;}
.gcm-hdr-close:hover{background:var(--bdr);}
...
.gcm-dna-btn{width:100%;padding:8px;border-radius:9px;border:1.5px dashed var(--bdr2);background:none;color:var(--tx2);font-size:12px;font-weight:700;cursor:pointer;font-family:var(--font);transition:all .15s;display:flex;align-items:center;justify-content:center;gap:5px;margin-bottom:4px;}
.gcm-dna-btn:hover{border-color:var(--a);color:var(--a);background:var(--a10);}
.gcm-dna-analyze{width:100%;padding:7px;border-radius:8px;border:none;background:rgba(139,92,246,.1);color:#7c3aed;font-size:11px;font-weight:700;cursor:pointer;font-family:var(--font);transition:background .15s;}
...
.gpm-tab{flex:1;padding:5px 6px;font-size:10px;font-weight:800;cursor:pointer;background:var(--surf2);color:var(--tx3);border:none;font-family:var(--font);transition:all .15s;text-align:center;}
...
.gpm-chip .gpm-del{opacity:0;font-size:11px;transition:opacity .12s;}
...
.gpm-addbtn{padding:6px 10px;border-radius:8px;border:none;background:var(--a);color:#fff;font-size:14px;cursor:pointer;flex-shrink:0;}
...
.gpm-legend .dot-b{width:8px;height:8px;border-radius:50%;background:#3b82f6;display:inline-block;margin-right:2px;}
.gpm-legend .dot-g{width:8px;height:8px;border-radius:50%;background:#f59e0b;display:inline-block;margin-right:2px;}
/* 공용멘트 체크박스 */
.gpm-global-check{display:flex;align-items:center;gap:8px;padding:7px 10px;border-radius:9px;border:1.5px solid var(--bdr2);background:var(--surf2);cursor:pointer;transition:all .18s;user-select:none;margin-top:5px;}
.gpm-global-check:hover{border-color:var(--a);background:var(--a10);}
.gpm-global-check.checked{border-color:#f59e0b;background:linear-gradient(135deg,rgba(254,243,199,.6),rgba(253,230,138,.3));}
.gpm-global-check input[type=checkbox]{width:16px;height:16px;accent-color:var(--a);cursor:pointer;flex-shrink:0;}
.gpm-global-check-lbl{flex:1;font-size:12px;font-weight:600;color:var(--tx2);}
.gpm-global-check.checked .gpm-global-check-lbl{color:#92400e;}
.gpm-global-cnt{font-size:10px;font-weight:800;padding:2px 7px;border-radius:10px;background:var(--a);color:#fff;flex-shrink:0;}
.gpm-global-check.checked .gpm-global-cnt{background:#f59e0b;}
.gpm-global-hint{font-size:10px;color:var(--tx3);line-height:1.4;padding:4px 10px;border-radius:0 0 8px 8px;border:1px solid var(--bdr);border-top:none;background:var(--surf2);margin-top:-2px;display:none;}
...
.gcm-book-slider{position:absolute;cursor:pointer;inset:0;background:var(--bdr2);border-radius:20px;transition:background .2s;}
.gcm-book-slider::before{content:'';position:absolute;height:14px;width:14px;left:3px;bottom:3px;background:#fff;border-radius:50%;transition:transform .2s;}
.gcm-book-toggle input:checked+.gcm-book-slider{background:var(--a);}
...
.gcm-btn-variants{background:linear-gradient(135deg,#f59e0b,#d97706);color:#fff;}
...
.gcm-vapply{margin-top:8px;width:100%;padding:9px;border-radius:10px;border:none;background:var(--a);color:#fff;font-size:13px;font-weight:800;cursor:pointer;font-family:var(--font);}
...
.gcm-cancel{padding:11px 20px;border-radius:11px;border:1.5px solid var(--bdr2);background:none;color:var(--tx2);font-size:13px;font-weight:700;cursor:pointer;font-family:var(--font);}
.gcm-save{flex:1;padding:11px 20px;border-radius:11px;border:none;background:var(--a);color:#fff;font-size:14px;font-weight:900;cursor:pointer;font-family:var(--font);box-shadow:0 3px 12px var(--a40);}
.gcm-save:disabled{opacity:.4;cursor:not-allowed;}
/* 문체 분석 결과 패널 */
/* 분석 패널 — 왼쪽 ctrl 내부에 표시 (오른쪽과 겹침 없음) */
.gcm-analyze-panel{display:none;border-top:1.5px solid var(--bdr);margin-top:4px;}
.gcm-analyze-panel.show{display:block;}
.gcm-analyze-hdr{display:flex;align-items:center;justify-content:space-between;padding:8px 0 4px;}
.gcm-analyze-title{font-size:9px;font-weight:800;letter-spacing:.7px;color:#7c3aed;text-transform:uppercase;}
.gcm-analyze-close{font-size:13px;cursor:pointer;color:var(--tx3);padding:2px 4px;line-height:1;border:none;background:none;font-family:var(--font);}
.gcm-analyze-close:hover{color:var(--tx);}
.gcm-analyze-body{padding:0 0 6px;font-size:11px;line-height:1.7;color:var(--tx2);white-space:pre-wrap;max-height:180px;overflow-y:auto;background:rgba(139,92,246,.06);border-radius:8px;padding:8px 10px;}
.gcm-analyze-body::-webkit-scrollbar{width:3px;}
.gcm-analyze-body::-webkit-scrollbar-thumb{background:rgba(139,92,246,.3);border-radius:2px;}
/* ══ BULK COMMENT MODAL ══════════════════════════════════ */
.gbk-overlay{position:fixed;inset:0;background:rgba(0,0,0,.55);backdrop-filter:blur(3px);z-index:2100;display:flex;align-items:center;justify-content:center;padding:12px;}
.gbk-modal{background:var(--card);border-radius:22px;width:100%;max-width:900px;max-height:94vh;box-shadow:0 20px 60px rgba(0,0,0,.28);display:flex;flex-direction:column;overflow:hidden;}
...
.gbk-hdr-texts{flex:1;min-width:0;}.gbk-hdr-title{font-size:16px;font-weight:900;color:var(--tx);}
.gbk-hdr-sub{font-size:12px;color:var(--tx3);margin-top:2px;}
.gbk-hdr-close{width:32px;height:32px;border-radius:50%;border:none;background:var(--surf2);cursor:pointer;font-size:15px;color:var(--tx2);display:flex;align-items:center;justify-content:center;}
.gbk-cfg{padding:10px 20px;border-bottom:1.5px solid var(--bdr);flex-shrink:0;display:flex;flex-wrap:wrap;gap:10px;align-items:flex-start;background:var(--surf);}
...
.gbk-inp-sm{padding:6px 10px;border-radius:8px;border:1.5px solid var(--bdr);background:var(--surf2);font-size:12px;color:var(--tx);font-family:var(--font);outline:none;min-width:160px;}
.gbk-prefix-wrap{width:100%;display:flex;flex-direction:column;gap:5px;padding-top:8px;border-top:1px solid var(--bdr);}
.gbk-prefix-lbl{font-size:10px;font-weight:800;color:var(--tx3);letter-spacing:.5px;text-transform:uppercase;}
.gbk-prefix-lbl span{background:#fef3c7;color:#92400e;padding:1px 7px;border-radius:10px;font-size:9px;font-weight:700;}
.gbk-prefix-ta{width:100%;box-sizing:border-box;padding:9px 12px;border-radius:9px;border:1.5px solid var(--bdr);background:var(--surf2);font-size:12px;line-height:1.6;color:var(--tx);font-family:var(--font);resize:none;outline:none;}
...
.gbk-table thead th{position:sticky;top:0;z-index:3;background:var(--surf2);border-bottom:1.5px solid var(--bdr);padding:9px 14px;text-align:left;font-size:11px;font-weight:800;color:var(--tx3);}
.gbk-table th:first-child{width:32px;text-align:center;padding:9px 8px;}
.gbk-tr{border-bottom:1px solid var(--bdr);}.gbk-tr:hover{background:var(--a10);}.gbk-tr-simple{border-bottom:1px solid var(--bdr);}.gbk-tr-simple:hover{background:var(--a10);cursor:default;}
.gbk-tr.selected{background:rgba(99,102,241,.04);}.gbk-tr.skip{opacity:.4;}
...
.gbk-status-badge{display:inline-flex;align-items:center;gap:4px;padding:3px 9px;border-radius:20px;font-size:10px;font-weight:700;}
...
.gbk-cmt-preview{font-size:12px;line-height:1.7;color:var(--tx2);padding:8px 10px;border-radius:8px;background:var(--surf2);border:1px solid var(--bdr);cursor:pointer;min-height:36px;word-break:break-word;}
...
.gbk-progress-track{height:6px;border-radius:6px;background:var(--bdr);overflow:hidden;margin-top:5px;}
...
/* ── 일괄 옵션 카드 ──────────────────────────────────── */
.gbk-opt-grid{display:grid;grid-template-columns:1fr 1fr;gap:8px;width:100%;}
@media(max-width:560px){.gbk-opt-grid{grid-template-columns:1fr;}}
.gbk-opt-card{
...
.gbk-opt-card:hover{border-color:var(--a);background:var(--a10);}
.gbk-opt-card input[type=checkbox]{width:15px;height:15px;accent-color:var(--a);cursor:pointer;flex-shrink:0;margin-top:2px;}
.gbk-opt-card-texts{flex:1;min-width:0;}
.gbk-opt-card-title{font-size:12px;font-weight:700;color:var(--tx);margin-bottom:3px;}
.gbk-opt-card-desc{font-size:10px;color:var(--tx3);line-height:1.5;}
...
.gbk-opt-card:hover{border-color:var(--a);background:var(--a10);}
.gbk-opt-card.on{border-color:var(--a);background:var(--a10);}
.gbk-opt-card input[type=checkbox]{width:16px;height:16px;accent-color:var(--a);cursor:pointer;flex-shrink:0;margin-top:2px;}
.gbk-opt-card-texts{flex:1;min-width:0;}
.gbk-opt-card-title{font-size:12px;font-weight:700;color:var(--tx);line-height:1.3;}
.gbk-opt-card.on .gbk-opt-card-title{color:var(--a);}
...
.gbk-dna-view-btn:hover{background:rgba(139,92,246,.16);}
/* 학생 행 — 체크박스 없는 심플 버전 */
.gbk-tr-simple{border-bottom:1px solid var(--bdr);transition:background .1s;}
.gbk-tr-simple:hover{background:var(--a10);}
...
.gbk-dna-panel-title{font-size:10px;font-weight:800;letter-spacing:.7px;color:var(--tx3);text-transform:uppercase;margin-bottom:8px;}
...
.gbk-dna-card{padding:10px 12px;border-radius:10px;border:1.5px solid var(--bdr);background:var(--surf2);font-size:11px;line-height:1.6;color:var(--tx2);}
...
/* ── 트렌드 뷰 ── */
.gr-trend-wrap{display:flex;flex-direction:column;height:100%;overflow-y:auto;padding:16px 18px 0;}
.gr-trend-header{margin-bottom:12px;flex-shrink:0;}
...
.gr-trend-growth{font-size:11px;padding:2px 9px;border-radius:12px;font-weight:700;}
.gr-trend-growth.up{background:rgba(34,197,94,.12);color:#16a34a;}
.gr-trend-growth.dn{background:rgba(239,68,68,.12);color:#dc2626;}
.gr-trend-growth.eq{background:rgba(148,163,184,.12);color:#64748b;}
.gr-trend-subtitle{font-size:11px;color:var(--tx3);margin-top:2px;}
/* 2컬럼 바디 */
.gr-trend-body{display:flex;gap:14px;flex:1;min-height:0;overflow:hidden;}
.gr-trend-left{flex:1;min-width:0;display:flex;flex-direction:column;gap:10px;overflow-y:auto;padding-bottom:16px;}
.gr-trend-right{width:256px;flex-shrink:0;display:flex;flex-direction:column;gap:8px;overflow-y:auto;padding-bottom:16px;}
/* 차트 */
.gr-trend-chart-outer{overflow-x:auto;overflow-y:hidden;border-radius:10px;border:1px solid var(--bdr);background:var(--surf);flex-shrink:0;}
.gr-trend-chart-area{height:280px;position:relative;}
#gr-trend-canvas{display:block;}
.gr-trend-legend{display:flex;flex-wrap:wrap;gap:8px 16px;flex-shrink:0;}
.gr-trend-leg{font-size:11px;font-weight:500;color:var(--tx2);display:flex;align-items:center;gap:5px;}
.gr-trend-leg-line{display:inline-block;width:20px;height:3px;border-radius:2px;vertical-align:middle;}
.gr-trend-leg-dash{display:inline-block;width:20px;border-top:2px dashed currentColor;vertical-align:middle;}
/* 교재 카드 그리드 */
.gr-tcard-section-title{font-size:11px;font-weight:700;color:var(--tx3);letter-spacing:.4px;text-transform:uppercase;flex-shrink:0;}
.gr-tcard-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(154px,1fr));gap:8px;flex-shrink:0;}
.gr-tcard{background:var(--surf);border:1.5px solid var(--bdr);border-radius:10px;padding:10px 12px;cursor:pointer;transition:border-color .15s,box-shadow .15s;}
.gr-tcard:hover{border-color:var(--a);box-shadow:0 0 0 2px var(--a20);}
.gr-tcard.sel{border-color:var(--a);box-shadow:0 0 0 3px var(--a20);background:var(--a10,var(--a20));}
.gr-tcard-top{display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:6px;gap:4px;}
.gr-tcard-name{font-size:12px;font-weight:700;color:var(--tx);line-height:1.3;flex:1;}
.gr-tcard-date{font-size:10px;color:var(--tx3);flex-shrink:0;white-space:nowrap;}
.gr-tcard-row{display:flex;align-items:center;gap:5px;margin-top:5px;}
.gr-tcard-lbl{font-size:10px;color:var(--tx3);width:26px;flex-shrink:0;}
.gr-tcard-bar-wrap{flex:1;height:5px;background:var(--surf2);border-radius:4px;overflow:hidden;}
.gr-tcard-bar{height:100%;border-radius:4px;transition:width .4s;}
.gr-tcard-pct{font-size:11px;font-weight:700;min-width:32px;text-align:right;flex-shrink:0;}
.gr-tcard-diff{font-size:10px;font-weight:700;min-width:30px;text-align:right;flex-shrink:0;}
.gr-tcard-diff.pos{color:#16a34a;}
.gr-tcard-diff.neg{color:#dc2626;}
.gr-tcard-no-avg{font-size:10px;color:var(--tx3);text-align:right;min-width:30px;}
/* AI 패널 */
.gr-trend-ai-box{background:var(--surf);border:1px solid var(--bdr);border-radius:10px;padding:12px 14px;display:flex;flex-direction:column;gap:8px;}
.gr-trend-ai-hdr{font-size:11px;font-weight:800;color:var(--tx3);letter-spacing:.4px;display:flex;align-items:center;justify-content:space-between;}
.gr-trend-ai-refresh{background:none;border:none;cursor:pointer;font-size:14px;color:var(--tx3);padding:0;line-height:1;transition:transform .3s;}
.gr-trend-ai-refresh:hover{transform:rotate(180deg);}
.gr-trend-ai-loading{display:flex;flex-direction:column;align-items:center;gap:10px;padding:20px 0;color:var(--tx3);font-size:12px;}
@keyframes gr-spin{to{transform:rotate(360deg)}}
.gr-trend-ai-spinner{width:22px;height:22px;border:2.5px solid var(--bdr);border-top-color:var(--a);border-radius:50%;animation:gr-spin .8s linear infinite;}
.gr-trend-ai-summary{font-size:12px;line-height:1.7;color:var(--tx2);border-left:3px solid var(--a);padding-left:10px;}
.gr-trend-ai-section{margin-top:4px;}
.gr-trend-ai-sec-title{font-size:10px;font-weight:800;color:var(--tx3);text-transform:uppercase;letter-spacing:.4px;margin-bottom:4px;}
.gr-trend-ai-item{font-size:11px;color:var(--tx2);line-height:1.6;display:flex;gap:5px;align-items:flex-start;}
.gr-trend-ai-direction{margin-top:6px;font-size:11px;font-weight:600;color:var(--a);line-height:1.5;background:var(--a20);border-radius:7px;padding:7px 10px;}
.gr-trend-ai-error{font-size:11px;color:var(--tx3);text-align:center;padding:16px 0;}
.gr-trend-no-data{text-align:center;padding:60px 20px;color:var(--tx3);font-size:13px;}`;
  ...
  let _trendSelIdx = null;           // 선택된 교재 카드 인덱스
  let _lastTrendPts = [];            // 마지막 계산된 트렌드 포인트
  const _trendAiCache = {};          // { studentId: analysisObj }
  ...
  function _renderTrend(cnt, studentId) {
    ...
    /* 성장률 배지 계산 (첫 교재 → 마지막 교재) */
    const clsName  = stu.classCode || '';
    let growthBadge = '';
    if (pts.length >= 2) {
      const first = pts[0], last = pts[pts.length - 1];
      const fv = first.wordAch ?? first.rdAch;
      const lv = last.wordAch  ?? last.rdAch;
      if (fv != null && lv != null) {
        const delta = lv - fv;
        const sign  = delta > 0 ? 'up' : delta < 0 ? 'dn' : 'eq';
        const icon  = delta > 0 ? '↑' : delta < 0 ? '↓' : '→';
        growthBadge = `<span class="gr-trend-growth ${sign}">${icon} ${delta > 0 ? '+' : ''}${delta}%</span>`;
      }
    }
    ...
    const stuHdr = `
      ...
      ${pts.length ? `<div class="gr-trend-subtitle">교재 평가일 순 · 카드 선택 시 차트 포인트 강조</div>` : ''}`;

    if (!pts.length) {
      cnt.innerHTML = `<div class="gr-trend-wrap">
    ...
    const legendHtml = `
      <div class="gr-trend-legend">
        ${hasWord ? `<span class="gr-trend-leg"><span class="gr-trend-leg-line" style="background:#3b82f6"></span>단어 성취율</span>` : ''}
        ${hasRd   ? `<span class="gr-trend-leg"><span class="gr-trend-leg-line" style="background:#8b5cf6"></span>리딩 성취율</span>` : ''}
        ${hasAvg && hasWord ? `<span class="gr-trend-leg" style="color:#93c5fd"><span class="gr-trend-leg-dash"></span>반 평균(단어)</span>` : ''}
        ${hasAvg && hasRd   ? `<span class="gr-trend-leg" style="color:#c4b5fd"><span class="gr-trend-leg-dash"></span>반 평균(리딩)</span>` : ''}
      </div>`;

    cnt.innerHTML = `
      <div class="gr-trend-wrap">
        <div class="gr-trend-header">${stuHdr}</div>
        <div class="gr-trend-body">
          <div class="gr-trend-left">
            ...
            ${legendHtml}
            <div class="gr-tcard-section-title">📋 교재별 평가 상세</div>
            <div class="gr-tcard-grid" id="gr-tcard-grid">
              ${_buildTrendCardsHtml(pts, _trendSelIdx)}
            </div>
          </div>
          <div class="gr-trend-right">
            <div class="gr-trend-ai-box" id="gr-trend-ai">
              <div class="gr-trend-ai-hdr">
                ✨ AI 성장 분석
                <button class="gr-trend-ai-refresh" onclick="GradeApp._refreshTrendAi('${studentId}')" title="재분석">↻</button>
              </div>
              <div class="gr-trend-ai-loading">
                <div class="gr-trend-ai-spinner"></div>
                <div>분석 중…</div>
              </div>
            </div>
          </div>
    ...
    requestAnimationFrame(() => {
      ...
      if (canvas && wrap) _drawTrendCanvas(canvas, wrap, pts, _trendSelIdx);
    });

    _loadTrendAi(studentId, pts);
  }

  /* ── 교재별 평가 카드 HTML ── */
  function _buildTrendCardsHtml(pts, selIdx) {
    return pts.map((p, i) => {
      const isSel  = i === selIdx;
      const wDiff  = p.wordAch != null && p.avgWord != null ? p.wordAch - p.avgWord : null;
      const rDiff  = p.rdAch   != null && p.avgRd   != null ? p.rdAch   - p.avgRd   : null;
      const diffHtml = d => {
        if (d == null) return `<span class="gr-tcard-no-avg">-</span>`;
        const cls = d >= 0 ? 'pos' : 'neg';
        return `<span class="gr-tcard-diff ${cls}">${d >= 0 ? '+' : ''}${d}%</span>`;
      };
      return `<div class="gr-tcard${isSel ? ' sel' : ''}" onclick="GradeApp._onTrendCard(${i})">
        <div class="gr-tcard-top">
          <span class="gr-tcard-name">${_e(p.bookName)}</span>
          <span class="gr-tcard-date">${p.date.slice(5)}</span>
        </div>
        ${p.wordAch != null ? `<div class="gr-tcard-row">
          <span class="gr-tcard-lbl">단어</span>
          <div class="gr-tcard-bar-wrap"><div class="gr-tcard-bar" style="width:${p.wordAch}%;background:#3b82f6"></div></div>
          <span class="gr-tcard-pct" style="color:#3b82f6">${p.wordAch}%</span>
          ${diffHtml(wDiff)}
        </div>` : ''}
        ${p.rdAch != null ? `<div class="gr-tcard-row">
          <span class="gr-tcard-lbl">리딩</span>
          <div class="gr-tcard-bar-wrap"><div class="gr-tcard-bar" style="width:${p.rdAch}%;background:#8b5cf6"></div></div>
          <span class="gr-tcard-pct" style="color:#8b5cf6">${p.rdAch}%</span>
          ${diffHtml(rDiff)}
        </div>` : ''}
      </div>`;
    }).join('');
  ...
  /* ── 카드 선택 → 차트 하이라이트 연동 ── */
  function _onTrendCard(idx) {
    _trendSelIdx = (_trendSelIdx === idx) ? null : idx;  // 토글
    // 카드 업데이트
    const grid = document.getElementById('gr-tcard-grid');
    if (grid) grid.innerHTML = _buildTrendCardsHtml(_lastTrendPts, _trendSelIdx);
    // 차트 재드로우
    const wrap   = document.getElementById('gr-trend-scroll');
    const canvas = document.getElementById('gr-trend-canvas');
    if (canvas && wrap) _drawTrendCanvas(canvas, wrap, _lastTrendPts, _trendSelIdx);
    // 선택 카드 스크롤
    if (_trendSelIdx != null && grid) {
      const cards = grid.querySelectorAll('.gr-tcard');
      cards[_trendSelIdx]?.scrollIntoView({ behavior: 'smooth', block: 'nearest', inline: 'nearest' });
    }
  }
  ...
  async function _loadTrendAi(studentId, pts) {
    ...
    try {
      ...
      const data = {
        ...
        books: pts.map(p => ({
          book: p.bookName, date: p.date,
          wordAch: p.wordAch, rdAch: p.rdAch,
          avgWord: p.avgWord, avgRd: p.avgRd,
        })),
      };
      ...
      if (el.isConnected) {
        el.innerHTML = `<div class="gr-trend-ai-hdr">✨ AI 성장 분석
          <button class="gr-trend-ai-refresh" onclick="GradeApp._refreshTrendAi('${studentId}')" title="재시도">↻</button>
        </div>
        <div class="gr-trend-ai-error">분석 실패 — ↻ 버튼으로 재시도</div>`;
      }
    }
  }
  ...
  function _refreshTrendAi(studentId) {
    ...
    if (el) el.innerHTML = `<div class="gr-trend-ai-hdr">✨ AI 성장 분석
      ...
      <div class="gr-trend-ai-loading"><div class="gr-trend-ai-spinner"></div><div>재분석 중…</div></div>`;
  }

  function _renderAiPanel(el, result, studentId) {
    if (!el?.isConnected) return;
    const trendIcon = result.trend === 'improving' ? '📈' : result.trend === 'declining' ? '📉' : '📊';
    el.innerHTML = `
      <div class="gr-trend-ai-hdr">${trendIcon} AI 성장 분석
        <button class="gr-trend-ai-refresh" onclick="GradeApp._refreshTrendAi('${studentId}')" title="재분석">↻</button>
      </div>
      <div class="gr-trend-ai-summary">${_e(result.summary || '')}</div>
      ${result.strengths?.length ? `
        <div class="gr-trend-ai-section">
          <div class="gr-trend-ai-sec-title">💪 강점</div>
          ${result.strengths.map(s => `<div class="gr-trend-ai-item"><span>✓</span><span>${_e(s)}</span></div>`).join('')}
        </div>` : ''}
      ${result.improvements?.length ? `
        <div class="gr-trend-ai-section">
          <div class="gr-trend-ai-sec-title">🔧 개선 포인트</div>
          ${result.improvements.map(s => `<div class="gr-trend-ai-item"><span>→</span><span>${_e(s)}</span></div>`).join('')}
        </div>` : ''}
      ${result.direction ? `<div class="gr-trend-ai-direction">🎯 ${_e(result.direction)}</div>` : ''}`;
  }
    ...
    for (const { classId, bookId, records } of trend) {
      ...
      if (classId && classId !== '__noclass__') {
        ...
        if (hasRd && clsRecs.length) {
          const rs = clsRecs.map(r => _calcRdN(r.reading||{}, actRevs)).filter(v=>v!=null);
  ...
  /* ── Canvas 꺾은선 차트 (선택 포인트 하이라이트 지원) ── */
  function _drawTrendCanvas(canvas, wrap, pts, selIdx) {
    ...
    const PAD  = { top: 32, right: 26, bottom: 86, left: 48 };
    ...
    const MONTH_C  = 'rgba(99,102,241,.05)';
    ...
    pts.forEach((p, i) => {
      ...
      if (!fill) return;
      const x0 = i === 0   ? PAD.left : (xOf(i-1)+xOf(i))/2;
      const x1 = i === n-1 ? PAD.left+cW : (xOf(i)+xOf(i+1))/2;
      ctx.fillStyle = MONTH_C;
      ctx.fillRect(x0, PAD.top, x1-x0, cH);
    });

    /* 선택 포인트 수직 강조선 */
    if (selIdx != null && selIdx < n) {
      const x = xOf(selIdx);
      ctx.fillStyle = 'rgba(99,102,241,.08)';
      ctx.fillRect(x - 24, PAD.top, 48, cH);
    ...
    function drawLine(getter, color, dashed) {
      ...
      if (!dashed && vpts.length > 1) {
        ctx.beginPath();
        vpts.forEach(({i,v},pi) => pi===0 ? ctx.moveTo(xOf(i),yOf(v)) : ctx.lineTo(xOf(i),yOf(v)));
      ...
      if (!dashed) {
        vpts.forEach(({i,v}) => {
          ...
          ctx.strokeStyle = color; ctx.lineWidth = isSel ? 3 : 2.5; ctx.stroke();
          ctx.fillStyle  = color;
          ctx.font       = isSel ? 'bold 12px sans-serif' : 'bold 10px sans-serif';
          ctx.textAlign  = 'center'; ctx.textBaseline = 'bottom';
          ctx.fillText(v+'%', x, y - r - 4);
        });
      } else {
        vpts.forEach(({i,v}) => {
    ...
    pts.forEach((p,i) => {
      const x = xOf(i), y = PAD.top+cH+10;
      const isSel = i === selIdx;
      if (isSel) { ctx.fillStyle = '#6366f1'; ctx.font = 'bold 11px sans-serif'; }
      else        { ctx.fillStyle = '#64748b'; ctx.font = '11px sans-serif'; }
      ctx.save();
      ctx.translate(x, y);
  ...
  function _captureOpts(bg) {
    return {
      ...
      onclone: (clonedDoc) => {
        // ★ 복제 DOM에서 transform scale 완전 제거 (배율 설정 무시하고 원본 크기 캡처)
        const clonedOuter = clonedDoc.getElementById('gr-rpt-outer');
        if (clonedOuter) {
          clonedOuter.style.transform = 'none';
          clonedOuter.style.transformOrigin = '';
        }
        // 폰트 링크 동기화
  ...
  // ── 캡처 대상 요소의 scale을 임시 해제하고 캡처 후 복원 ──
  async function _captureEl(el, bg) {
    const outer = document.getElementById('gr-rpt-outer');
    const prevTransform = outer ? outer.style.transform : '';
    const prevOrigin    = outer ? outer.style.transformOrigin : '';
    // ★ 캡처 전 scale 임시 제거
    if (outer) { outer.style.transform = 'none'; outer.style.transformOrigin = ''; }
    await new Promise(r => requestAnimationFrame(() => requestAnimationFrame(r)));
    try {
      const canvas = await html2canvas(el, _captureOpts(bg));
      return canvas;
    } finally {
      // ★ 캡처 후 scale 복원
      if (outer) { outer.style.transform = prevTransform; outer.style.transformOrigin = prevOrigin; }
  ...
  async function _deliverReport() {
    ...
    try {
      await _waitFonts();
      const canvas = await _captureEl(el, _st.rptBg);
...
<div id="page-center">
  ...
  async function _printReport(){
    ...
    if(typeof html2canvas!=='undefined'){
      ...
      await _waitFonts();
      _captureEl(el).then(canvas=>{
        const imgUrl=canvas.toDataURL('image/png');
        const pw={A4:'210mm',A5:'148mm',B5:'176mm',Letter:'216mm',Card:'105mm',Narrow:'127mm',Wide:'210mm'}[_st.pageSize]||'210mm';
        const ph={A4:'297mm',A5:'210mm',B5:'250mm',Letter:'279mm',Card:'148mm',Narrow:'180mm',Wide:'297mm'}[_st.pageSize]||'297mm';
  ...
  async function _deliverOverview() {
    ...
    try {
      await _waitFonts();
      const canvas = await _captureEl(el, _ov.cfg.bg||'#ffffff');
  ...
  function _openCommentPop(e, sid) {
    ...
    <div class="gcm-modal">
      <div class="gcm-hdr">
        ...
        <div class="gcm-hdr-info">
          <div class="gcm-hdr-title">Teacher's Comment</div>
          <div class="gcm-hdr-sub">${_e(stuName)}${bkName ? ' · ' + _e(bkName) : ''}</div>
        </div>
        <button class="gcm-hdr-close" id="gcm-close-btn">✕</button>
      </div>
      <div class="gcm-body">
        <div class="gcm-ctrl">
          <div>
            <div class="gcm-sec-title">🧬 Style DNA</div>
            <div style="font-size:11px;color:var(--tx3);margin-bottom:8px">직접 쓴 코멘트를 저장하면 AI가 같은 문체로 생성합니다.</div>
          ...
          <div>
            ...
            <div class="gpm-tabs">
              ...
              <button class="gpm-tab" id="gcm-tab-global">🌐 공용</button>
            </div>
            <label class="gpm-global-check" id="gcm-global-check">
              <input type="checkbox" id="gcm-global-use">
              <span class="gpm-global-check-lbl">🌐 공용 멘트도 AI에 포함</span>
              <span class="gpm-global-cnt" id="gcm-global-cnt">0개</span>
            </label>
            ...
            <div class="gpm-legend">
              ...
              <span><span class="dot-g"></span>🌐 공용</span>
            </div>
          </div>
          <div>
            <div class="gcm-sec-title">📚 교재 상태</div>
            <div style="display:flex;align-items:center;gap:7px;margin-bottom:6px">
              <label class="gcm-book-toggle"><input type="checkbox" id="gcm-book-done"><span class="gcm-book-slider"></span></label>
              <span style="font-size:12px;color:var(--tx2);font-weight:600">이번 교재 이수 완료</span>
            </div>
            <div id="gcm-next-wrap" style="display:none;margin-top:6px">
              <input class="gcm-inp-sm" id="gcm-next-book" placeholder="다음 교재명 (선택)">
            </div>
          </div>
          <div>
            <div class="gcm-sec-title">💬 선생님 메모</div>
            <textarea class="gcm-memo" id="gcm-memo" rows="3" placeholder="특별히 언급할 내용…"></textarea>
          </div>
          <div>
            <div class="gcm-sec-title">✨ AI 생성</div>
            <div style="display:flex;flex-direction:column;gap:7px">
              <button class="gcm-btn gcm-btn-gen" id="gcm-btn-gen">✨ AI 코멘트 생성</button>
              <button class="gcm-btn gcm-btn-variants" id="gcm-btn-variants">🎲 3가지 버전으로 생성</button>
              <button class="gcm-btn gcm-btn-proof" id="gcm-btn-proof">🔍 문법 교정</button>
            </div>
          </div>
          <!-- 문체 분석 결과 — 왼쪽 패널 하단 (오른쪽 variants와 겹침 없음) -->
          <div class="gcm-analyze-panel" id="gcm-analyze-panel">
            <div class="gcm-analyze-hdr">
              <span class="gcm-analyze-title">🔍 내 문체 분석 결과</span>
              <button class="gcm-analyze-close" id="gcm-analyze-close" type="button">✕</button>
            </div>
            <div class="gcm-analyze-body" id="gcm-analyze-body"></div>
          </div>
        </div>
        <div class="gcm-edit">
          <div class="gcm-edit-top">
            <textarea class="gcm-ta" id="gcm-ta" placeholder="코멘트를 직접 입력하거나 AI 생성을 시작하세요…"></textarea>
            <div class="gcm-ta-meta">
              <span class="gcm-char" id="gcm-char">0자</span>
              <button class="gcm-save-sample" id="gcm-save-sample">📎 스타일 샘플로 저장</button>
            </div>
          </div>
          <div class="gcm-status idle" id="gcm-status">AI 대기 중</div>
          <div class="gcm-variants" id="gcm-variants">
            <div style="font-size:11px;font-weight:800;color:var(--tx3);letter-spacing:.7px;margin-bottom:8px;text-transform:uppercase">✦ 버전을 선택하세요</div>
            <div class="gcm-vlist" id="gcm-vlist"></div>
            <button class="gcm-vapply" id="gcm-vapply" disabled>선택한 버전 적용</button>
          </div>
          <div class="gcm-footer">
            <button class="gcm-cancel" id="gcm-cancel">취소</button>
            <button class="gcm-save" id="gcm-save">💾 저장</button>
          </div>
        </div>
      </div>
    </div>`;
    ...
    const statusEl=overlay.querySelector('#gcm-status'), pinList=overlay.querySelector('#gcm-pin-list');
    const pinInp=overlay.querySelector('#gcm-pin-inp'), dnaCount=overlay.querySelector('#gcm-dna-count');
    const varPanel=overlay.querySelector('#gcm-variants'), vList=overlay.querySelector('#gcm-vlist');
    const vApply=overlay.querySelector('#gcm-vapply'), bookDone=overlay.querySelector('#gcm-book-done');
    const nextWrap=overlay.querySelector('#gcm-next-wrap'), nextBook=overlay.querySelector('#gcm-next-book');
    const memoEl=overlay.querySelector('#gcm-memo');
    const tabBook=overlay.querySelector('#gcm-tab-book'), tabGlobal=overlay.querySelector('#gcm-tab-global');
    const gcmGU=overlay.querySelector('#gcm-global-use'), gcmGC=overlay.querySelector('#gcm-global-check');
    const gcmGN=overlay.querySelector('#gcm-global-cnt');
    ...
    let _busy=false;
    const _lock=on=>{_busy=on;['gcm-btn-gen','gcm-btn-variants','gcm-btn-proof','gcm-save','gcm-dna-analyze'].forEach(id=>{const b=overlay.querySelector('#'+id);if(b)b.disabled=on;});};
    const _curBookId=()=>_st.bookId||'';
    const _updateDna=()=>{const c=(typeof GeminiAI!=='undefined')?GeminiAI.getStyleSamples().length:0;dnaCount.textContent='샘플 '+c+'개 학습됨';};
    const _updateGCnt=()=>{const n=(typeof GeminiAI!=='undefined')?GeminiAI.getPins().length:0;if(gcmGN)gcmGN.textContent=n+'개';};
    let _pinTab='book'; const _activeMap={book:new Set(),global:new Set()};
    const _renderPins=()=>{
      ...
      pins.forEach((p,i)=>{
        ...
        chip.onclick=ev=>{
          if(ev.target.classList.contains('gpm-del')){if(isBook)GeminiAI.removeBookPin(bid,i);else GeminiAI.removePin(i);actSet.delete(i);_renderPins();_updateGCnt();return;}
          if(actSet.has(i))actSet.delete(i);else actSet.add(i);_renderPins();
        };
        pinList.appendChild(chip);
      });
    };
    const _getActivePins=()=>{
      if(typeof GeminiAI==='undefined')return[];
    ...
    const _addPin=()=>{if(typeof GeminiAI==='undefined')return;const v=pinInp.value.trim();if(!v)return;const bid=_curBookId();if(_pinTab==='book'){if(GeminiAI.addBookPin(bid,v))_activeMap.book.add(GeminiAI.getBookPins(bid).length-1);}else{if(GeminiAI.addPin(v)){_activeMap.global.add(GeminiAI.getPins().length-1);_updateGCnt();}}pinInp.value='';_renderPins();};
    overlay.querySelector('#gcm-pin-add-btn').onclick=_addPin;
    pinInp.onkeydown=ev=>{if(ev.key==='Enter'){ev.preventDefault();_addPin();}};
    overlay.querySelector('#gcm-dna-save').onclick=()=>{if(typeof GeminiAI==='undefined'||!ta.value.trim())return;GeminiAI.addStyleSample(ta.value.trim());_updateDna();_setStatus('ok','✓ 스타일 샘플로 저장됨');setTimeout(()=>_setStatus('idle','AI 대기 중'),2500);};
    overlay.querySelector('#gcm-save-sample').onclick=()=>{if(typeof GeminiAI==='undefined'||!ta.value.trim())return;GeminiAI.addStyleSample(ta.value.trim());_updateDna();_setStatus('ok','✓ 스타일 샘플 저장 완료');setTimeout(()=>_setStatus('idle','AI 대기 중'),2500);};
    // 분석 패널 참조
    const analyzePanel = overlay.querySelector('#gcm-analyze-panel');
    const analyzeBody  = overlay.querySelector('#gcm-analyze-body');
    const analyzeClose = overlay.querySelector('#gcm-analyze-close');
    if (analyzeClose) analyzeClose.onclick = () => { if(analyzePanel) analyzePanel.classList.remove('show'); };
    overlay.querySelector('#gcm-dna-analyze').onclick=async()=>{
      ...
      try{
        ...
        _setStatus('ok','✓ 분석 완료 — 아래 결과를 확인하세요');
        if(analyzeBody) analyzeBody.textContent=r;
        if(analyzePanel){ analyzePanel.classList.add('show'); const ctrl=overlay.querySelector('.gcm-ctrl'); if(ctrl)ctrl.scrollTop=ctrl.scrollHeight; }
    ...
    overlay.querySelector('#gcm-btn-gen').onclick=async()=>{
      ...
      try{const info={name:stu?stu.name:sid,word:d.word,reading:d.reading};ta.value=await GeminiAI.generateComment(info,_getBookStatus(),{prevComments:prevRecs,activePins:_getActivePins()});charEl.textContent=ta.value.length+'자';_setStatus('ok','✓ 생성 완료');}
      catch(err){_setStatus('err','⚠ '+_cpMsg(err));}finally{_lock(false);}
    };
    overlay.querySelector('#gcm-btn-variants').onclick=async()=>{
      if(typeof GeminiAI==='undefined'){_setStatus('err','⚠ GeminiAI 미로드');return;}
      _lock(true);_setStatus('loading','3가지 버전 생성 중…');vList.innerHTML='';varPanel.classList.remove('show');vApply.disabled=true;
      try{
        const info={name:stu?stu.name:sid,word:d.word,reading:d.reading};
        const vars=await GeminiAI.generateVariants(info,_getBookStatus(),{prevComments:prevRecs,activePins:_getActivePins()},3);
        let selIdx=-1;
        vars.forEach((v,i)=>{const card=document.createElement('div');card.className='gcm-vcard';card.innerHTML='<span class="gcm-vcard-badge">선택됨</span>'+_e(v);card.onclick=()=>{vList.querySelectorAll('.gcm-vcard').forEach(c=>c.classList.remove('selected'));card.classList.add('selected');selIdx=i;vApply.disabled=false;};vList.appendChild(card);});
        varPanel.classList.add('show');
        vApply.onclick=()=>{if(selIdx<0)return;ta.value=vars[selIdx];charEl.textContent=ta.value.length+'자';varPanel.classList.remove('show');_setStatus('ok','✓ 선택 버전 적용됨');};
    ...
    overlay.querySelector('#gcm-save').onclick=async()=>{
      ...
      const val=ta.value.trim();_st.data[sid].comment=val;_st.dirty.add(sid);_refreshDirtyUI();
  ...
  function _closeCommentPop() {
    ...
    const old=document.getElementById('gr-cmt-modal');if(old)old.remove();
    if(_activeCmtPop&&_activeCmtPop!==el){if(_activeCmtPop._keyH)document.removeEventListener('keydown',_activeCmtPop._keyH);if(_activeCmtPop._clickH)document.removeEventListener('click',_activeCmtPop._clickH);_activeCmtPop.remove();}
    _activeCmtPop=null;
  }
  ...
  function _openBulkComment() {
    ...
    const stuData = students.map(s => {
      ...
      const wd = rec ? (rec.word    || {}) : {};
      const rd = rec ? (rec.reading || {}) : {};
    ...
    const _results = {};
    stuData.forEach(s => {
      _results[s.id] = { comment: s.existingComment, status: s.existingComment ? 'has' : 'wait' };
    });

    // ── 헬퍼 ──────────────────────────────────────────────
    const _e2 = v => String(v ?? '').replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/"/g,'&quot;');
    const _pill = (pct, lbl) => {
    ...
    const _badge = r => {
      ...
      if (r.status === 'err')      return '<span class="gbk-status-badge err" title="' + _e2(r.err||'') + '">⚠ 오류</span>';
    ...
    // ── 모달 생성 ─────────────────────────────────────────
    const overlay = document.createElement('div');
    overlay.className = 'gbk-overlay';
    overlay.id = 'gbk-overlay';
    ...
    <div class="gbk-modal">
      <div class="gbk-hdr">
        ...
        <div class="gbk-hdr-texts">
          <div class="gbk-hdr-title">AI 일괄 코멘트 생성</div>
          <div class="gbk-hdr-sub">${_e(bookName)} · 총 ${students.length}명</div>
        </div>
        <button class="gbk-hdr-close" id="gbk-close" type="button">✕</button>
      </div>
      <div class="gbk-cfg">
        <div class="gbk-opt-grid">
          <label class="gbk-opt-card on" id="gbk-opt-skip-lbl">
            <input type="checkbox" id="gbk-skip-existing" checked>
            <div class="gbk-opt-card-texts">
              <div class="gbk-opt-card-title">✍️ 이미 작성된 코멘트 유지</div>
              <div class="gbk-opt-card-desc">코멘트가 있는 학생은 건너뛰고<br>빈 학생만 새로 생성합니다.</div>
            </div>
          </label>
          <label class="gbk-opt-card" id="gbk-opt-done-lbl">
            <input type="checkbox" id="gbk-book-done">
            <div class="gbk-opt-card-texts">
              <div class="gbk-opt-card-title">🎓 이번 교재 이수 완료 포함</div>
              <div class="gbk-opt-card-desc">교재 이수 완료 내용을 코멘트에<br>자연스럽게 녹여 넣습니다.</div>
            </div>
          </label>
        </div>
        <div id="gbk-next-wrap" style="display:none;width:100%;margin-top:4px">
          <input class="gbk-inp-sm" id="gbk-next-book" placeholder="📚 다음 교재명 입력" style="width:100%;box-sizing:border-box">
        </div>
        <div style="width:100%;display:flex;flex-direction:column;gap:5px;padding-top:8px;border-top:1px solid var(--bdr)">
          <div style="display:flex;align-items:center;gap:7px;flex-wrap:wrap">
            <span style="font-size:11px;font-weight:700;color:var(--tx2)">📝 공통 앞말</span>
            <span style="font-size:10px;color:var(--tx3)">모든 학생 코멘트 맨 앞에 동일하게 추가됩니다. 비워두면 AI가 바로 작성합니다.</span>
          </div>
          <textarea class="gbk-prefix-ta" id="gbk-prefix" rows="2" placeholder="예) 4월 16일 배부된 교재입니다. (비워두면 이 부분 없이 생성됩니다)"></textarea>
        </div>
        <div style="width:100%;display:flex;gap:7px;padding-top:6px;border-top:1px solid var(--bdr);flex-wrap:wrap;align-items:center">
          <button id="gbk-pin-toggle" type="button" style="padding:5px 12px;border-radius:8px;border:1.5px solid var(--bdr2);background:var(--surf2);color:var(--tx2);font-size:11px;font-weight:700;cursor:pointer;display:inline-flex;align-items:center;gap:5px;font-family:var(--font);white-space:nowrap">
            📌 고정 멘트 설정 <span id="gbk-pin-cnt" style="background:var(--a);color:#fff;border-radius:10px;padding:1px 6px;font-size:10px">0</span>
          </button>
          <div class="gbk-dna-popup-wrap" id="gbk-dna-popup-wrap">
            <button class="gbk-dna-view-btn" id="gbk-dna-view-btn" type="button">🧬 스타일 분석 결과 보기</button>
            <div class="gbk-dna-popup" id="gbk-dna-popup">
              <div class="gbk-dna-popup-title">🧬 선생님 Style DNA <span class="gbk-dna-popup-close" id="gbk-dna-popup-close">✕</span></div>
              <div id="gbk-dna-result" class="gbk-dna-popup-empty">아직 분석 결과가 없습니다. 🧬 스타일 학습 후 생성을 눌러주세요.</div>
            </div>
          </div>
        </div>
      </div>
      <div class="gbk-pin-mgr" id="gbk-pin-mgr" style="display:none">
        ...
        <div class="gpm-tabs">
          <button class="gpm-tab on" id="gbk-tab-book" type="button">🏫 이 교재 전용</button>
          <button class="gpm-tab" id="gbk-tab-global" type="button">🌐 공용</button>
        </div>
        ...
        <div class="gpm-global-hint" id="gbk-global-hint">체크하면 공용 멘트가 모든 학생 코멘트에 반영됩니다.</div>
        <div class="gpm-chip-wrap" id="gbk-pin-list"></div>
        <div class="gpm-add">
          <input class="gpm-inp" id="gbk-pin-inp" placeholder="🏫 예) 발음이 좋아졌어요…" maxlength="40">
          <button class="gpm-addbtn" id="gbk-pin-addbtn" type="button">＋</button>
        </div>
        <div class="gpm-legend">
          <span><span class="dot-b"></span>🏫 이 교재 전용</span>
          <span><span class="dot-g"></span>🌐 공용</span>
      ...
      <div class="gbk-body">
        <div class="gbk-table-wrap">
          <table class="gbk-table">
            <thead><tr>
              <th style="min-width:90px;padding:9px 14px">학생</th>
              <th style="width:100px;text-align:center">단어 / 리딩</th>
              <th style="width:70px;text-align:center">상태</th>
              <th>코멘트 <span style="font-size:9px;font-weight:400;color:var(--tx3)">(클릭하여 수정)</span></th>
            </tr></thead>
      ...
      <div class="gbk-footer">
        <div class="gbk-footer-left">
          <button class="gbk-btn gbk-btn-start" id="gbk-btn-start" type="button">✨ 일괄 생성</button>
          <button class="gbk-btn gbk-btn-stop"  id="gbk-btn-stop"  type="button" style="display:none">⏹ 중단</button>
          <button class="gbk-btn gbk-btn-style" id="gbk-btn-style" type="button">🧬 스타일 학습 후 생성</button>
          <button class="gbk-btn gbk-btn-proof" id="gbk-btn-proof" type="button" disabled>🔍 일괄 교정</button>
        </div>
        <div class="gbk-footer-right">
          <button class="gbk-btn gbk-btn-save"   id="gbk-btn-save"   type="button" disabled>💾 전체 저장</button>
          <button class="gbk-btn gbk-btn-cancel" id="gbk-btn-cancel" type="button">닫기</button>
        </div>
      </div>
    </div>`;
    ...
    const nextWrap   = overlay.querySelector('#gbk-next-wrap');
    ...
    const gbkGU      = overlay.querySelector('#gbk-global-use');
    const gbkGC      = overlay.querySelector('#gbk-global-check');
    const gbkGN      = overlay.querySelector('#gbk-global-cnt');
    const gbkGH      = overlay.querySelector('#gbk-global-hint');
    const dnaPopup   = overlay.querySelector('#gbk-dna-popup');
    const dnaViewBtn = overlay.querySelector('#gbk-dna-view-btn');
    const dnaClose   = overlay.querySelector('#gbk-dna-popup-close');
    ...
    overlay.addEventListener('click', ev => {
      if (ev.target === overlay && !_running) _closeBulk();
    });

    // ── 옵션 카드 시각 토글 ───────────────────────────────
    const _optCard = (lblId, chk) => {
      const lbl = overlay.querySelector('#' + lblId);
      if (lbl) lbl.classList.toggle('on', chk.checked);
    };
    if (skipExist)  skipExist.onchange  = () => _optCard('gbk-opt-skip-lbl', skipExist);
    if (bookDoneCk) bookDoneCk.onchange = () => {
      _optCard('gbk-opt-done-lbl', bookDoneCk);
      if (nextWrap) nextWrap.style.display = bookDoneCk.checked ? '' : 'none';
    };
    ...
    document.addEventListener('click', function _dnaOutside(ev) {
      if (!overlay.isConnected) { document.removeEventListener('click', _dnaOutside); return; }
      if (dnaPopup && !dnaPopup.contains(ev.target) && ev.target !== dnaViewBtn) dnaPopup.classList.remove('show');
    });
    ...
    const _gbkUpdateCnt  = () => { if (pinCnt) pinCnt.textContent = _gbkGetActive().length; };
    const _gbkUpdateGCnt = () => {
      if (typeof GeminiAI === 'undefined') return;
      const n = GeminiAI.getPins().length;
      if (gbkGN) gbkGN.textContent = n + '개';
      if (gbkGC) gbkGC.style.opacity = n ? '1' : '.5';
    };
    const _gbkRenderPins = () => {
      ...
      if (!pins.length) {
        ...
        pins.forEach((p, i) => {
          const chip = document.createElement('span');
          chip.className = 'gpm-chip' + (actSet.has(i) ? (isBook ? ' book-active' : ' active') : '');
          chip.innerHTML = _e2(p) + ' <span class="gpm-del" title="삭제">✕</span>';
          chip.onclick = ev => {
            if (ev.target.classList.contains('gpm-del')) {
              if (isBook) GeminiAI.removeBookPin(bid, i); else GeminiAI.removePin(i);
              actSet.delete(i); _gbkRenderPins(); _gbkUpdateCnt(); _gbkUpdateGCnt(); return;
            }
    ...
    if (typeof GeminiAI !== 'undefined' && GeminiAI.loadPinsFromDB) GeminiAI.loadPinsFromDB().then(() => { _gbkUpdateGCnt(); _gbkUpdateCnt(); });
    if (gbkGU) { gbkGU.checked = false; if (typeof GeminiAI !== 'undefined') GeminiAI.setUseGlobalPins(false); }
    _gbkUpdateGCnt();
    if (gbkGU) gbkGU.onchange = () => {
      const on = gbkGU.checked;
      if (typeof GeminiAI !== 'undefined') GeminiAI.setUseGlobalPins(on);
      if (gbkGC) gbkGC.classList.toggle('checked', on);
      if (gbkGH) gbkGH.classList.toggle('show', on);
      _gbkUpdateCnt();
    };
    ...
    if (pinToggle) pinToggle.onclick = () => {
      ...
      if (show) { _gbkRenderPins(); _gbkUpdateGCnt(); }
    };
    const gbkAddBtn = overlay.querySelector('#gbk-pin-addbtn');
    const _gbkAddPin = () => {
      if (typeof GeminiAI === 'undefined' || !gbkPinInp) return;
      const v = gbkPinInp.value.trim(); if (!v) return;
      const bid = _gbkBookId();
      if (_gbkPinTab === 'book')   { if (GeminiAI.addBookPin(bid, v)) _gbkActMap.book.add(GeminiAI.getBookPins(bid).length - 1); }
      else { if (GeminiAI.addPin(v)) { _gbkActMap.global.add(GeminiAI.getPins().length - 1); _gbkUpdateGCnt(); } }
    ...
    const _updateRowStatus = sid => {
      ...
      if (cmtCell && overlay._editingSid !== sid) {
        ...
        cmtCell.innerHTML = v
          ? '<div class="gbk-cmt-preview" onclick="GradeApp._bulkEditCell(\'' + sid + '\')">'+_e2(v)+'</div>'
    ...
    const _renderTable = () => {
      ...
      stuData.forEach(sd => {
        ...
        tr.innerHTML =
          '<td class="gbk-td" style="vertical-align:middle"><div class="gbk-stu-name">'+_e2(sd.name)+'</div>'+(sd.nickname ? '<div class="gbk-stu-nick">('+_e2(sd.nickname)+')</div>' : '')+'</td>'+
          ...
          '<td class="gbk-td gbk-td-cmt"  id="gbk-cmt-'+sd.id+'">'+
            (res.comment
              ? '<div class="gbk-cmt-preview" onclick="GradeApp._bulkEditCell(\'' +sd.id+ '\')">'+_e2(res.comment)+'</div>'
    ...
    // ── 공통 생성 헬퍼 ────────────────────────────────────
    const _setGenerating = on => {
      _running = on;
      btnStart.disabled = on; btnProof.disabled = on; btnStyle.disabled = on;
      btnStop.style.display = on ? '' : 'none';
      btnSave.disabled = on;
      overlay.querySelector('#gbk-close').disabled = on;
    };
    const _runBulk = async (targets, extraMemo) => {
      if (!targets.length) { progTxt.textContent = '생성할 대상이 없습니다.'; return; }
      const prefix    = (prefixEl ? prefixEl.value : '').trim();
      const isCompl   = bookDoneCk ? bookDoneCk.checked : false;
      const nextBook  = nextBookIn  ? nextBookIn.value.trim()  : '';
      ...
      for (const sd of targets) {
        ...
        try {
          ...
          console.error('[BulkGen]', sd.name, _results[sd.id].err);
        }
    ...
    btnStart.onclick = async () => {
      if (_running) return;
      _stopFlag = false; _setGenerating(true); progWrap.style.display = '';
      btnStart.style.display = 'none';
      const doSkip  = skipExist ? skipExist.checked : false;
      const targets = stuData.filter(sd => !(doSkip && _results[sd.id].comment && _results[sd.id].status === 'has'));
      await _runBulk(targets, '');
      _running = false; _setGenerating(false);
      btnStart.style.display = ''; btnStart.textContent = '🔄 다시 생성';
    ...
    btnProof.onclick = async () => {
      if (_running) return;
      const targets = stuData.filter(sd => _results[sd.id].comment && ['done','has'].includes(_results[sd.id].status));
      ...
      for (const sd of targets) {
        ...
        try {
          ...
          _results[sd.id].comment = corrected; _results[sd.id].status = 'done';
      ...
      btnSave.disabled = false; _refreshDirtyUI();
    };

    // ── 스타일 학습 후 생성 ───────────────────────────────
    btnStyle.onclick = async () => {
      if (_running || typeof GeminiAI === 'undefined') return;
      ...
      if (!unique.length) { _toast('⚠ 분석할 코멘트가 없습니다. 먼저 일부 학생 코멘트를 입력하거나 스타일 샘플을 저장해 주세요.'); return; }
      if (dnaPopup) dnaPopup.classList.add('show');
      if (dnaResult) { dnaResult.className = 'gbk-dna-popup-empty'; dnaResult.textContent = '⟳ 선생님 문체를 분석 중입니다…'; }
      unique.slice(0, 10).forEach(c => GeminiAI.addStyleSample(c));
      let analysisText = '';
      try {
        analysisText = await GeminiAI.analyzeStyle();
        if (dnaResult) { dnaResult.className = 'gbk-dna-popup-body'; dnaResult.textContent = analysisText; }
        if (dnaViewBtn) dnaViewBtn.style.background = 'rgba(139,92,246,.2)';
      } catch(err) {
        if (dnaResult) { dnaResult.className = 'gbk-dna-popup-empty'; dnaResult.textContent = '⚠ 분석 실패: ' + String(err && err.message ? err.message : err).slice(0, 60); }
      }
      _stopFlag = false; _setGenerating(true); progWrap.style.display = '';
      const memo = analysisText ? '[스타일 가이드] ' + analysisText.slice(0, 200) : '';
      await _runBulk(stuData.slice(), memo);
      _running = false; _setGenerating(false);
      const ok = Object.values(_results).filter(r => r.status === 'done').length;
      const er = Object.values(_results).filter(r => r.status === 'err').length;
    ...
    // ── 버튼 이벤트 ──────────────────────────────────────
    btnCancel.onclick = () => _closeBulk();
    overlay.querySelector('#gbk-close').onclick = () => _closeBulk();
    overlay._keyH = ev => { if (ev.key === 'Escape' && !_running) _closeBulk(); };
    document.addEventListener('keydown', overlay._keyH);
  }
  function _bulkEditCell(sid) {
    ...
    if(prevSid&&prevSid!==sid){const prevTa=document.getElementById('gbk-ta-'+prevSid);if(prevTa){const v=prevTa.value.trim();if(overlay._results){overlay._results[prevSid].comment=v;overlay._results[prevSid].status=v?'done':'wait';}const cell=document.getElementById('gbk-cmt-'+prevSid);if(cell)cell.innerHTML=v?'<div class="gbk-cmt-preview" onclick="GradeApp._bulkEditCell(\''+prevSid+'\')">'+_e(v)+'</div>':'<div class="gbk-cmt-preview empty" onclick="GradeApp._bulkEditCell(\''+prevSid+'\')"> (클릭하여 직접 입력…)</div>';}}
    overlay._editingSid=sid;const res=overlay._results?.[sid]||{},cmtCell=document.getElementById('gbk-cmt-'+sid);if(!cmtCell)return;
