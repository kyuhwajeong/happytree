...
|**🗂 콘텐츠(자료실)**|파일 자료실 + 🎬 영상 워크시트 + 📝 PDF 워크시트 제작 + 🎮 학습 게임 4종 통합 탭|
...
|**캡처/PDF**|html2canvas v1.4.1, jsPDF v2.5.1, pdf.js v3.11.174, **pdf-lib v1.17.1**(PDF 워크시트 제작 도구, 신규)|
...
|Cloudflare Worker 업로드 토큰|`archive-db.js` (`UPLOAD\_TOKEN`)|🔴 높음|presign(업로드) + **삭제(DELETE)** 권한까지 있는 Bearer 토큰. 유출 시 자료실 파일을 임의로 올리거나 지울 수 있음|
...
(반·계정·테마)(교재·챕터)(성적·설정)(Gemini API) (직원DB)  (자료+영상+게임 데이터)
   ...
   bg-theme.js(배경 이미지, theme.bg 경로 공유)
           ...
           └── monitor/        히든 모니터링
               ...
               ├── fcm\_tokens/ FCM 디바이스 토큰
               └── ip\_labels/  IP 대역 장소명 라벨
...
│                           · 섹션 순서 커스터마이즈(헤더 ≡ 버튼, hk10b\_dashboardOrder)
...
│
├── pdf-editor-app.js          PDF 워크시트 제작 (콘텐츠 탭 하위 도구, 신규 발견·문서화)
│                           · 여러 PDF/이미지를 페이지 단위로 자르고 합쳐 새 워크시트 PDF 제작
│                           · 텍스트·이미지 주석(annots) 편집, pdf-lib(내보내기)+pdf.js(미리보기) 병행
│                           · 완성본 다운로드 + 콘텐츠 자료실(파일)에 자동 등록, 별도 DB 없음
│
...
├── monitor-db.js       v5.2  세션 추적 + IP 지오코딩 + 🧹원격 명령 채널
│                           · ip-api.com 한국어 위치 조회
│                           · IP 대역 장소명 라벨 관리
├── monitor-app.js      v4.2  히든 모니터링 대시보드 UI
│                           · 브라우저 OS 알림 (새 세션 감지)
│                           · 이상 접속 플래그 (심야/중복/과다)
│                           · 통계 대시보드 탭 (48h 집계)
│                           · 메뉴 사용 히트맵 (요일×시간대)
│                           · 🧹 원격 캐시 전체 삭제 버튼
├── monitor-patch.js    v5.2  전 메뉴 액션 추적 (monkey-patch)
│                           · PdfEditorApp 신규 커버리지(원본 추가·저장→자료실 등록)
│
├── guest-mode.js       v3.2  게스트 나레이션 데모 시스템
│                           · SVG 마스크 스포트라이트 + 말풍선
│                           · 이전/다음/다시보기 스텝 내비게이션
│                           · 탭·서브메뉴 자동 전환 (action 콜백)
│                           · 홈(dashboard)·콘텐츠(archive) 나레이션 신규 추가
│                           · Write 차단 화이트리스트에 4개 신규 DB 모듈 추가(보안 수정)
│                           · 콘텐츠 나레이션에 📝PDF 워크시트 제작 스텝 보강
│
...
│
├── progress/{classId\_\_{weekKey}\_\_{dayName}\_\_{bookId}\_\_progress}
│   └── 진도 텍스트         "p.32\~38" 형태
│
├── bookdata/{bookId}
...
```
파일명: "04.\[T2] 파닉스 몬스터 4\_20260516.xlsx"
         └번호┘└반┘ └────교재명────┘└──날짜──┘
...
|섹션 순서 변경|헤더 ≡ 버튼으로 드래그 재배치, 기기별 `hk10b\_dashboardOrder`에 저장|
...
#### 📝 PDF 워크시트 제작 (pdf-editor-app.js, 신규 발견·문서화)

|기능|설명|
|-|-|
|원본 추가|PDF·이미지 파일 여러 개 추가, 자료실에서 바로 가져오기도 가능|
|페이지 편집|페이지 단위 자르기·순서 재배열·빈 페이지 삽입, 텍스트/이미지 주석(annots) 얹기|
|실행취소/다시실행|Ctrl+Z / Ctrl+Y(Ctrl+Shift+Z), 그리드·편집기 공용|
|2쪽 모아찍기(N-up)|인쇄용으로 2쪽씩 한 장에 모아 내보내기 옵션|
|저장|완성본을 내 PC로 다운로드하는 동시에 **콘텐츠 자료실(파일)에 자동 등록**(제목·분류·공개범위 지정)|
|한글 폰트|사이트에 이미 있는 Noto Sans KR/나눔고딕 등 4종 재사용, pdf-lib에 별도 임베드 없이 캔버스 렌더링으로 처리|
|작업 중 보호|저장 안 된 페이지가 있으면 새로고침/닫기 시 브라우저 확인창 표시|

> ⚠️ \*\*모니터링/게스트모드 사각지대였음(2026-08 발견)\*\*: 이 도구는 archive-app.js에
> 4번째 도구로 이미 추가돼 있었지만, monitor-patch.js 추적 대상과 guest-mode.js
> 나레이션 어디에도 없었습니다. 이번 점검에서 둘 다 보강했습니다(§8, §9 참고).
...
```
KEY\_1 ──┐                     gemini-2.5-flash-lite (1순위)
KEY\_2 ──┼─ 429/401 자동 전환 → gemini-2.5-flash      (2순위)
KEY\_3 ──┘
```
...
|`monitor-patch.js` v5.2|전 메뉴(신규 7종 포함) 함수 monkey-patch → 액션 자동 기록, PdfEditorApp 신규 커버리지(신규)|
...
IP 라벨 (hakwon10/monitor/ip\_labels)
device / browser / OS
startedAt / lastSeen / loggedOut
...
### 추적 액션 범위 (monitor-patch.js v5.2)

> v5.1은 사용자 요청으로 v5.0에서 다룬 6개 신규 모듈의 전체 함수 목록을
> 다시 한번 전수 대조해 놓친 액션을 보강한 버전입니다. 진행 중 발견한 버그
> 2건도 함께 수정했습니다(하단 참고).
>
> \*\*v5.2\*\*: 공유 폴더 소스를 다시 대조하는 과정에서, `archive-app.js`의
> `TOOL\_TABS`에 4번째 도구로 이미 추가돼 있던 \*\*`pdf-editor`(PdfEditorApp,
> 📝 PDF 워크시트 제작)\*\*가 이 파일의 추적 대상 어디에도 없어 완전한
> 사각지대였음을 확인했습니다. 원본 PDF/이미지 추가와 저장(자료실 등록)
> 액션을 추가하고, 탭 전환 라벨 맵에도 `'pdf-editor'` 항목을 보강했습니다.
...
|콘텐츠·자료실|하위 탭 전환, 업로드, 열람, 삭제, 수정, 공유, zip 다운로드, 엑셀 편집, 파일 변환, 업로드 폼 열기·분류 추가/삭제·즐겨찾기·인쇄·비밀번호 보호 자료 열람 시도(v5.1), **📝 PDF 워크시트 제작 — 원본 PDF·이미지 추가, 저장(→자료실 자동 등록)**(v5.2)|
...
→ hakwon10/monitor/fcm\_tokens/{deviceId} 저장
...
* 🗂 콘텐츠 (archive) — **v3.1 신규**, **v3.2 보강**: 자료실·🎬영상 워크시트·📝PDF 워크시트 제작·🎮학습 게임 4탭을 순서대로 소개 (`\_selectTool` action으로 탭 자동 전환)

> ℹ️ 일정관리·공지사항은 별도 페이지가 아니라 홈/헤더에 내장된 기능이라
> 독립된 나레이션 페이지를 두지 않았습니다(홈 나레이션에서 함께 소개).

### 📝 v3.2 보강: PDF 워크시트 제작 나레이션 누락

공유 폴더 소스를 다시 대조하는 과정에서, `archive-app.js`의 콘텐츠 탭이 이미
4개 도구(자료실·영상 워크시트·**PDF 워크시트 제작**·학습 게임)로 확장돼
있었는데도, v3.1 콘텐츠 나레이션은 3개 도구만 소개하고 있었음을 확인했습니다
(제목도 "자료실·영상·게임"으로 표기). PDF 워크시트 제작을 소개하는 스텝을
추가하고 제목·강조 텍스트를 4개 도구 기준으로 정정했습니다.
...
|키|내용|
|-|-|
|`hk10b\_nav\_order`|하단 탭 순서 (pg 배열)|
|`hk10b\_rem\_id` / `hk10b\_rem\_pw`|아이디·비밀번호 기억하기|
|`hk10b\_cls`|반 목록 캐시|
|`hk10b\_prog`|진도 캐시|
|`hk10b\_acc`|계정 캐시|
|`hk10b\_theme`|테마 캐시|
|`hk10b\_sess`|로그인 세션|
|`hk\_fcm\_did`|FCM 디바이스 고유 ID|

### 교재 관리

|키|내용|
|-|-|
|`bl\_ch\_w`|챕터명 열 너비 (px)|
|`bl\_pf\_{classId}\_{bookId}`|후처리 필터 데이터|

### 성적 관리

|키|내용|
|-|-|
|`gr\_layout`|리포트 레이아웃 (1\~5)|
|`gr\_pageSize`|페이지 크기|
|`gr\_rptScale`|배율 (60\~150)|
|`gr\_fontFamily`|본문 폰트|
|`gr\_reportBold`|Bold 강조|
|`gr\_rptBg`|배경색|
|`gr\_titleSz` / `gr\_bodySz`|제목·본문 크기|
|`gr\_hdrFontSz` / `gr\_excelFontSz` / `gr\_cardFontSz`|뷰별 폰트 크기|
|`gr\_graph` / `gr\_graphStyle` / `gr\_graphAlign`|그래프 설정|
|`gr\_logoSz` / `gr\_titleAlign`|로고·제목 정렬|
|`gr\_tblHdrBg` / `gr\_tblHdrClr` / `gr\_tblCellBg` / `gr\_tblRound`|표 색상|
|`gr\_divClr` / `gr\_divW`|구분선|
|`gr\_ov\_horiz` / `gr\_ov\_bg` / `gr\_ov\_font` / `gr\_ov\_bold`|Overview 설정|

### Gemini AI

|키|내용|
|-|-|
|`ht\_style\_samples`|코멘트 스타일 샘플 (최대 20개)|
|`ht\_style\_pins`|공용 고정 멘트 (Firebase globalPins 동기화)|
|`ht\_style\_analysis`|스타일 분석 캐시|
|`ht\_style\_pins:{bookId}`|교재별 고정 멘트|

### 홈 대시보드 / 일정 / 공지 / 콘텐츠 (신규)

|키|내용|
|-|-|
|`hk10b\_dashboardOrder`|홈 대시보드 섹션 순서 (기기별)|
|`db\_live\_quote`|오늘의 명언 캐시 (24h)|
|`hk10b\_schedules`|일정 로컬 캐시|
|`sch\_weather\_cache\_v4`|날씨 예보 캐시 (위치명 포함)|
|`sch\_dismiss\_{scheduleId}` (sessionStorage)|일정 알림 팝업 "나중에" 임시 닫기|
|`hk10b\_notices`|공지 로컬 캐시|
|공지 알림 dismiss 키 (sessionStorage)|공지 팝업 "나중에" 임시 닫기|
|`hk10b\_archive`|자료실 게시물 로컬 캐시|
|`hk10b\_archiveCategories`|자료실 분류 목록 캐시|
|`hk10b\_archiveViewMode`|자료실 보기 모드(그리드/리스트 등)|
|`hk10b\_eduvideo`|교육 영상 로컬 캐시|
|`hk10b\_eduvideo\_topics`|교육 영상 주제 목록 캐시|
|`hk10b\_staff\_home`|직원 관리 화면 시작 탭 설정|
...
```
환경변수: FCM\_SERVICE\_ACCOUNT = Firebase 서비스 계정 JSON 전체
...
목록 로딩(FB\_PATH)과 완전히 분리된 별도 Firebase 경로(`archiveSearchIndex`)에
...
(guest-mode.js)를 다시 전수 점검함. 그 사이 `archive-app.js`의 `TOOL\_TABS`에
\*\*4번째 도구로 `pdf-editor`(PdfEditorApp, 📝 PDF 워크시트 제작)\*\*가 이미
추가돼 있었는데, 코드 자체는 정상 동작하지만 다음 두 곳에서 완전히 누락되어
있었음을 확인:

* **monitor-patch.js**: PdfEditorApp에 대한 추적(`\_wrap`)이 전혀 없어, 이
도구 안에서 무엇을 했는지(원본 추가, 저장 등) 모니터링 대시보드에 전혀
기록되지 않고 있었음. 탭 전환 라벨 맵(`ArchiveApp.\_selectTool`)에도
`'pdf-editor'` 항목이 없어 탭을 눌러도 라벨 없이 raw key로만 남았음.
→ v5.1 → **v5.2**: 원본 PDF·이미지 추가, 저장(자료실 등록) 액션 추가 +
탭 라벨 보강.
* **guest-mode.js**: 콘텐츠 나레이션이 "자료실·영상·게임" 3개 도구만
소개하고 있어 PDF 워크시트 제작 도구가 데모 투어에서 완전히 빠져 있었음.
→ v3.1 → **v3.2**: PDF 워크시트 제작 소개 스텝 추가, 제목 문구 정정.

이번 점검에서는 dev 브랜치에 별도 신규 커밋이 있던 게 아니라(마지막 관련
커밋은 `1afac72`), pdf-editor 기능 자체가 이전 v5.0/v5.1/v3.1 점검 시점보다
먼저 추가돼 있었는데도 그때 두 시스템 모두 대조 대상에서 빠졌던 것으로
확인됨. 함께 §2(pdf-lib 기술스택)·§4(파일 구성)·§6(콘텐츠 메뉴 설명)도
PDF 워크시트 제작 도구 기준으로 갱신함.
...
아닌 잘못된 키(`'library'`)를 참조해 라벨이 안 붙던 문제, `GameApp. \_selectSource` 라벨에 존재하지 않는 값(`'manual'`)을 매핑해두던 문제
...
08. 탭 순서        : localStorage('hk10b\_nav\_order') → \_renderNav()
