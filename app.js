...
const App = (() => {
  ...
  const LS_REM='hk9_remember_id';
  const S={page:'operate',mgTab:'classes',selCls:null,monday:_mon(new Date()),mgMk:DB.monthKey(new Date()),editClsId:null,editAccId:null,tmpTheme:null,viewMode:'grid',operateView:'grid',calY:new Date().getFullYear(),calM:new Date().getMonth(),shareActive:false};
  ...
  function _hideSplash(){const sp=document.getElementById('splash');sp.classList.add('out');setTimeout(()=>{sp.style.display='none';document.getElementById('app').classList.remove('hidden');go('operate');},380);}
  ...
  function _showLogin(){
    const savedId=localStorage.getItem(LS_REM);
    _q('li-id').value=savedId||'';_q('li-pw').value='';_q('li-err').textContent='';
    _q('li-remember').checked=!!savedId;
    _q('login-gate').classList.remove('hidden');
    setTimeout(()=>(savedId?_q('li-pw'):_q('li-id')).focus(),300);
  }
  ...
  function doLogin(){
    ...
    if(acc){
      if(_q('li-remember').checked)localStorage.setItem(LS_REM,id);else localStorage.removeItem(LS_REM);
  ...
  function logout(){if(!confirm('로그아웃 하시겠습니까?'))return;DB.clearSession();_refreshAuthUI();go('operate');_toast('로그아웃 되었습니다');}

  /* ── 운용 ── */
  function _renderChips(){
    const classes=DB.getClasses(),wrap=_q('op-chips');if(!wrap)return;wrap.innerHTML='';
    if(!classes.length){wrap.innerHTML='<span style="font-size:11px;color:var(--tx3);white-space:nowrap">관리 메뉴에서 반을 추가하세요</span>';return;}
    if(S.selCls&&!classes.find(c=>c.id===S.selCls.id))S.selCls=null;
    if(!S.selCls)S.selCls=classes[0];
    classes.forEach(cls=>{const b=document.createElement('button');b.className='chip'+(S.selCls?.id===cls.id?' on':'');b.textContent=cls.name;b.onclick=()=>{S.selCls=cls;_renderChips();_renderDays();};wrap.appendChild(b);});
  }
  ...
  function _renderDays(){
    ...
    DAYS.forEach((dayName,i)=>{
      ...
      if(!mainBooks.length&&!subBooks.length){card.innerHTML+='<div class="no-bk">이 월에 배정된 교재가 없습니다</div>';}
        ...
        const ta=document.createElement('textarea');ta.className='memo-inp';ta.rows=1;ta.placeholder='이 요일 메모 입력...';ta.value=memoVal;
  ...
  async function shareCurrentClass(){
    ...
    const sd={title:`${cls.name}반 진도 현황`,text:`${cls.name}반 이번 주(${_wom(S.monday)}주차) 수업 진도를 확인하세요.`,url};
  ...
  async function _copyUrl(url){try{await navigator.clipboard.writeText(url);_toast('🔗 링크 복사 완료! 카카오·문자 등으로 공유하세요','success',3000);}catch{prompt('링크를 복사하세요:',url);}}
  ...
  function _renderCal(){
    const yr=S.calY,mo=S.calM;_q('cal-title').textContent=`${yr}년 ${mo+1}월`;
    const grid=_q('cal-grid');grid.innerHTML='';
    const today=new Date();today.setHours(0,0,0,0);const selMon=_mon(S.monday);
    const startDow=new Date(yr,mo,1).getDay();const lastDay=new Date(yr,mo+1,0).getDate();
  ...
  function _renderManage(){
    ...
    _q('vt-l').classList.toggle('on',S.viewMode==='list');_q('vt-g').classList.toggle('on',S.viewMode==='grid');
  ...
  function mgTab(tab){
    S.mgTab=tab;const TABS=['classes','accounts','theme','io','share'];
    ...
    if(tab==='classes')_renderMgCls();if(tab==='accounts')_renderMgAcc();
    if(tab==='theme')_renderMgTheme();if(tab==='io')_renderMgIO();if(tab==='share')_renderMgShare();
  }

  function setView(mode){S.viewMode=mode;_q('vt-l').classList.toggle('on',mode==='list');_q('vt-g').classList.toggle('on',mode==='grid');const t=DB.getTheme();t.viewMode=mode;DB.saveTheme(t);_renderMgCls();}

  function _renderMgCls(){
    const wrap=_q('mg-classes');if(!wrap)return;wrap.innerHTML='';
    const isAdmin=DB.isAdmin();
    if(isAdmin){const btn=document.createElement('button');btn.className='add-cls';btn.innerHTML='<span style="font-size:18px">＋</span> 반 추가';btn.onclick=()=>openClassModal();wrap.appendChild(btn);}
    ...
    classes.forEach(cls=>cont.appendChild(_buildClsCard(cls,isAdmin)));wrap.appendChild(cont);
  }
  ...
  function _buildClsCard(cls,isAdmin){
    ...
    card.innerHTML=`<div class="cls-chdr"><div class="cls-chdr-l"><div class="cls-nm">${_esc(cls.name)}</div><div class="dbadges">${dayBadges}</div></div><div class="cls-chdr-r">${isAdmin?`<button class="ibtn" onclick="App.openClassModal('${cls.id}')" title="수정">✏️</button><button class="ibtn red" onclick="App.delClass('${cls.id}')" title="삭제">🗑</button>`:''}</div></div><div class="cls-mo-nav"><button onclick="App.mgPrev()" title="이전 달">‹</button><span class="cls-mo-lbl">📅 ${mkY}년 ${mkM}월 교재</span><button onclick="App.mgNext()" title="다음 달">›</button></div>`;
    ...
    card.appendChild(bm);return card;
  }
  ...
  function _buildPoolZone(clsId,mk,books,isAdmin){
    ...
    if(isAdmin&&(books.pool||[]).length){const cb=document.createElement('button');cb.className='clear-btn';cb.textContent='전체삭제';cb.onclick=async()=>{if(!confirm('교재 목록 전체 삭제?'))return;await DB.clearZone(clsId,mk,'pool');_toast('🗑 목록 삭제');};acts.appendChild(cb);}
    hdr.appendChild(acts);zone.appendChild(hdr);
    const list=document.createElement('div');list.className='bm-pool-list';list.dataset.zone='pool';list.dataset.clsid=clsId;list.dataset.mk=mk;
    (books.pool||[]).forEach(b=>list.appendChild(_buildPoolItem(b,clsId,mk,isAdmin)));
    ...
    if(isAdmin){
      ...
      const doAdd=async()=>{const name=inp.value.trim();if(!name){_toast('⚠️ 교재명을 입력해주세요','error');inp.focus();return;}await DB.addToPool(clsId,mk,name);inp.value='';inp.focus();_toast(`📚 "${name}" 추가 완료`,'success');};
      btn.onclick=doAdd;inp.onkeydown=e=>{if(e.key==='Enter')doAdd();};
      ar.appendChild(inp);ar.appendChild(btn);zone.appendChild(ar);
    }
    _setupDropZone(list,'pool',clsId,mk);return zone;
  }

  function _buildPoolItem(b,clsId,mk,isAdmin){
    const item=document.createElement('div');item.className='bm-pool-item';item.draggable=true;item.dataset.bookid=b.id;item.dataset.name=b.name;
    ...
    if(isAdmin){
      ...
      const toMain=document.createElement('button');toMain.className='bm-pool-btn to-main';toMain.title='주교재로';toMain.textContent='主';toMain.onclick=async(e)=>{e.stopPropagation();await DB.moveBook(clsId,mk,b.id,'main');_toast('📘 주교재로 이동','success');};
      const toSub=document.createElement('button');toSub.className='bm-pool-btn to-sub';toSub.title='부교재로';toSub.textContent='副';toSub.onclick=async(e)=>{e.stopPropagation();await DB.moveBook(clsId,mk,b.id,'sub');_toast('📗 부교재로 이동','success');};
      const del=document.createElement('button');del.className='bm-pool-btn del';del.title='삭제';del.textContent='✕';del.onclick=async(e)=>{e.stopPropagation();if(!confirm(`"${b.name}" 삭제?`))return;await DB.deleteBook(clsId,mk,b.id);_toast('🗑 삭제 완료');};
      btns.appendChild(toMain);btns.appendChild(toSub);btns.appendChild(del);item.appendChild(btns);
    }
    _setupDragItem(item,b.id,b.name,'pool',clsId,mk);return item;
  }
  ...
  function _buildAssignZones(clsId,mk,books,isAdmin){
    ...
    ['main','sub'].forEach(zone=>{
      ...
      if(isAdmin){
        ...
        arBtn.onclick=async()=>{const sel=document.querySelector('.bm-pool-item.selected');if(!sel){_toast('⚠️ 교재 목록에서 교재를 먼저 선택하세요','error');return;}await DB.moveBook(sel.dataset.clsid||clsId,mk,sel.dataset.bookid,zone);_toast(`${zone==='main'?'📘 주':'📗 부'}교재로 이동`,'success');};
        acts.appendChild(arBtn);
        if((books[zone]||[]).length){const cb=document.createElement('button');cb.className='clear-btn';cb.textContent='전체삭제';cb.onclick=async()=>{if(!confirm(`${zone==='main'?'주':'부'}교재 전체 삭제?`))return;await DB.clearZone(clsId,mk,zone);_toast('🗑 전체 삭제');};acts.appendChild(cb);}
      }
      hdr.appendChild(acts);zDiv.appendChild(hdr);
      const list=document.createElement('div');list.className='bm-zone-list';list.dataset.zone=zone;list.dataset.clsid=clsId;list.dataset.mk=mk;
      (books[zone]||[]).forEach(b=>{
        const item=document.createElement('div');item.className='bm-zone-item';item.draggable=isAdmin;item.dataset.bookid=b.id;item.dataset.name=b.name;
        ...
        if(isAdmin){const back=document.createElement('button');back.className='bm-back-btn';back.title='목록으로';back.textContent='↩';back.onclick=async(e)=>{e.stopPropagation();await DB.moveBook(clsId,mk,b.id,'pool');_toast('↩ 목록으로 이동');};item.appendChild(back);_setupDragItem(item,b.id,b.name,zone,clsId,mk);}
      ...
      _setupDropZone(list,zone,clsId,mk);zDiv.appendChild(list);right.appendChild(zDiv);
    });
    return right;
  }
  ...
  function _setupDragItem(el,bookId,name,fromZone,clsId,mk){
    el.addEventListener('dragstart',e=>{_drag={item:el,bookId,name,fromZone,clsId,mk};el.classList.add('dragging');e.dataTransfer.effectAllowed='move';e.dataTransfer.setData('text/plain',bookId);});
    ...
    el.addEventListener('touchstart',e=>{const t=e.touches[0];_drag={item:el,bookId,name,fromZone,clsId,mk};el.classList.add('dragging');const ghost=_q('drag-ghost');ghost.textContent=name;ghost.classList.remove('hidden');ghost.style.left=t.clientX+'px';ghost.style.top=t.clientY+'px';_drag.ghost=ghost;},{passive:true});
    el.addEventListener('touchmove',e=>{const t=e.touches[0];if(_drag.ghost){_drag.ghost.style.left=t.clientX+'px';_drag.ghost.style.top=t.clientY+'px';}document.querySelectorAll('.drop-hover').forEach(z=>z.classList.remove('drop-hover'));const under=document.elementFromPoint(t.clientX,t.clientY);const zoneEl=under?.closest('.bm-zone-list,.bm-pool-list');if(zoneEl)zoneEl.classList.add('drop-hover');},{passive:true});
    el.addEventListener('touchend',async e=>{const t=e.changedTouches[0];if(_drag.ghost)_drag.ghost.classList.add('hidden');el.classList.remove('dragging');document.querySelectorAll('.drop-hover').forEach(z=>z.classList.remove('drop-hover'));const under=document.elementFromPoint(t.clientX,t.clientY);const zoneEl=under?.closest('.bm-zone-list,.bm-pool-list');if(zoneEl){const toZone=zoneEl.dataset.zone;if(toZone&&toZone!==_drag.fromZone){await DB.moveBook(_drag.clsId,_drag.mk,_drag.bookId,toZone);_toast(`${toZone==='main'?'📘 주교재':toZone==='sub'?'📗 부교재':'📚 목록'}으로 이동`,'success');}}if(_drag.fromZone==='pool'){document.querySelectorAll('.bm-pool-item.selected').forEach(x=>{if(x!==el)x.classList.remove('selected');});el.classList.toggle('selected');}});
    if(fromZone==='pool')el.addEventListener('click',()=>{document.querySelectorAll('.bm-pool-item.selected').forEach(x=>{if(x!==el)x.classList.remove('selected');});el.classList.toggle('selected');});
  }
  ...
  function _setupDropZone(el,zone,clsId,mk){
    ...
    el.addEventListener('drop',async e=>{e.preventDefault();el.classList.remove('drop-hover');const bookId=e.dataTransfer.getData('text/plain');if(bookId&&zone!==_drag.fromZone){await DB.moveBook(_drag.clsId||clsId,_drag.mk||mk,bookId,zone);_toast(`${zone==='main'?'📘 주교재':zone==='sub'?'📗 부교재':'📚 목록'}으로 이동`,'success');}});
  }

  function mgPrev(){S.mgMk=DB.prevMonthKey(S.mgMk);_renderMgCls();}
  function mgNext(){S.mgMk=DB.nextMonthKey(S.mgMk);_renderMgCls();}

  function openClassModal(id=null){S.editClsId=id;const cls=id?DB.getClassById(id):null;_q('mcls-t').textContent=id?'반 수정':'반 추가';_q('f-cname').value=cls?.name||'';document.querySelectorAll('#modal-cls .day-ck input').forEach(cb=>{cb.checked=cls?(cls.days||[]).includes(cb.value):false;});_q('modal-cls').classList.remove('hidden');}
  async function saveClass(){const name=_q('f-cname').value.trim();if(!name){_toast('⚠️ 반 이름을 입력해주세요','error');return;}if(!S.editClsId&&DB.classExists(name)){_toast(`⚠️ "${name}" 반이 이미 존재합니다`,'error');return;}const days=[...document.querySelectorAll('#modal-cls .day-ck input:checked')].map(c=>c.value);if(!days.length){_toast('⚠️ 요일을 선택해주세요','error');return;}if(S.editClsId){await DB.updateClass(S.editClsId,{name,days});if(S.selCls?.id===S.editClsId)S.selCls=DB.getClassById(S.editClsId);_toast('✅ 반 수정 완료','success');}else{const r=await DB.addClass({name,days});if(!r){_toast('⚠️ 반 추가 실패','error');return;}_toast('✅ 반 추가 완료','success');}closeModal('cls');_renderMgCls();_renderChips();}
  async function delClass(id){const cls=DB.getClassById(id);if(!confirm(`"${cls?.name}" 반을 삭제하시겠습니까?\n모든 진도 데이터도 삭제됩니다.`))return;await DB.deleteClass(id);if(S.selCls?.id===id)S.selCls=null;_renderMgCls();_renderChips();_toast('🗑 삭제 완료');}

  function _renderMgAcc(){const wrap=_q('mg-accounts');if(!wrap)return;wrap.innerHTML='';const isAdmin=DB.isAdmin(),sess=DB.getSession();if(isAdmin){const b=document.createElement('button');b.className='add-cls';b.style.marginBottom='6px';b.innerHTML='<span>＋</span> 계정 추가';b.onclick=()=>openAccModal();wrap.appendChild(b);}const note=document.createElement('div');note.style.cssText='font-size:11px;color:var(--tx2);margin-bottom:8px;line-height:1.65;padding:8px 10px;background:var(--card2);border-radius:var(--rs)';note.innerHTML='<b style="color:var(--tx)">admin</b>: 관리메뉴 전체 + 진도입력<br><b style="color:var(--tx)">operator</b>: 운용화면 진도 입력만';wrap.appendChild(note);const card=document.createElement('div');card.className='acc-card';DB.getAccounts().forEach(acc=>{const isMe=sess?.id===acc.id,row=document.createElement('div');row.className='acc-row';row.innerHTML=`<div><div class="acc-nm">${_esc(acc.username)}${isMe?'&nbsp;<span style="color:var(--green);font-size:10px">●</span>':''}<span class="role-badge ${acc.role}">${acc.role==='admin'?'관리자':'운용자'}</span></div><div class="acc-role">${acc.role==='admin'?'모든 기능':'진도 입력만'}</div></div><div class="acc-acts">${isAdmin?`<button class="ibtn" onclick="App.openAccModal('${acc.id}')">✏️</button>`:''}${isAdmin&&!isMe?`<button class="ibtn red" onclick="App.delAcc('${acc.id}','${_esc(acc.username)}')">🗑</button>`:''}</div>`;card.appendChild(row);});wrap.appendChild(card);}
  ...
  function _renderMgTheme(){
    ...
    const t=DB.getTheme();S.tmpTheme={...t};const isAdmin=DB.isAdmin();
    const card=document.createElement('div');card.className='th-card';
    const prev=document.createElement('div');prev.className='th-row';prev.innerHTML='<div class="th-preview" id="th-prev"></div>';card.appendChild(prev);
    ...
    FONTS.forEach(f=>{const item=document.createElement('div');item.className='ff-item'+(f.key===(t.fontFamily||'Noto Sans KR')?' on':'');item.style.fontFamily=`'${f.key}',sans-serif`;item.innerHTML=`<span class="ff-name">${f.label}</span><span class="ff-sample">${f.sample}</span>`;if(!isAdmin){item.style.pointerEvents='none';item.style.opacity='.45';}item.onclick=()=>{S.tmpTheme.fontFamily=f.key;_applyTheme(S.tmpTheme);ffList.querySelectorAll('.ff-item').forEach((el,i)=>el.classList.toggle('on',FONTS[i].key===f.key));};ffList.appendChild(item);});
    ...
    const ipWrap=document.createElement('div');ipWrap.className='inp-preview-wrap';ipWrap.innerHTML=`<div class="inp-preview-box"><span class="bk-tag main" style="font-size:9px;padding:2px 6px;border-radius:4px">주</span><span class="inp-preview-nm">수학의 정석(상)</span><div class="inp-preview-inp" id="inp-prev-box" style="width:${t.inputBoxWidth||140}px">p.123~130</div></div>`;
    ...
    [{v:'grid',l:'⊞ 그리드'},{v:'list',l:'☰ 리스트'}].forEach(({v,l})=>{const btn=document.createElement('button');btn.className='view-sel-btn'+(v===(t.operateView||'grid')?' on':'');btn.textContent=l;if(!isAdmin){btn.disabled=true;btn.style.opacity='.45';}btn.onclick=()=>{S.tmpTheme.operateView=v;S.operateView=v;vrow.querySelectorAll('.view-sel-btn').forEach((b,i)=>b.classList.toggle('on',['grid','list'][i]===v));};vrow.appendChild(btn);});
    ovr.appendChild(vrow);card.appendChild(ovr);
    if(isAdmin){const sr=document.createElement('div');sr.className='th-row';const sb=document.createElement('button');sb.className='th-save-btn';sb.textContent='💾 테마 저장 · 적용';sb.onclick=async()=>{sb.textContent='저장 중...';sb.disabled=true;await DB.saveTheme(S.tmpTheme);_applyTheme(S.tmpTheme);S.operateView=S.tmpTheme.operateView||'grid';sb.textContent='💾 테마 저장 · 적용';sb.disabled=false;_toast('🎨 테마 저장 완료! 모든 기기에 반영됩니다.','success',3500);_renderMgTheme();};sr.appendChild(sb);card.appendChild(sr);}
    else{const nr=document.createElement('div');nr.className='th-row';nr.innerHTML='<div style="font-size:11px;color:var(--tx3)">⚠️ 테마 변경은 관리자 로그인 후 가능합니다</div>';card.appendChild(nr);}
  ...
  function _renderMgIO(){
    const wrap=_q('mg-io');if(!wrap)return;wrap.innerHTML='';const isAdmin=DB.isAdmin();
    const card=document.createElement('div');card.className='io-card';
    const exRow=document.createElement('div');exRow.className='io-row';exRow.innerHTML='<div><div class="io-title">📤 엑셀 내보내기</div><div class="io-desc">반·교재·진도 전체 백업</div></div>';
    const exBtn=document.createElement('button');exBtn.className='io-btn ex';exBtn.textContent='내보내기';exBtn.disabled=!isAdmin;exBtn.onclick=_exportExcel;exRow.appendChild(exBtn);card.appendChild(exRow);
    const imRow=document.createElement('div');imRow.className='io-row';imRow.innerHTML='<div><div class="io-title">📥 엑셀 불러오기</div><div class="io-desc">DB 초기화 후에도 복구 가능</div></div>';
    const imBtn=document.createElement('button');imBtn.className='io-btn im';imBtn.textContent='파일 선택';imBtn.disabled=!isAdmin;imBtn.onclick=()=>_q('xl-in').click();imRow.appendChild(imBtn);card.appendChild(imRow);
    wrap.appendChild(card);
    const drop=document.createElement('div');drop.className='drop-zone';drop.innerHTML='📂 엑셀 파일을 여기에 드래그하거나 탭하세요<br><span style="font-size:10px;margin-top:4px;display:block">PC·모바일 모두 지원</span>';
    drop.addEventListener('dragover',e=>{e.preventDefault();drop.classList.add('drag-over');});drop.addEventListener('dragleave',()=>drop.classList.remove('drag-over'));
    ...
    drop.addEventListener('click',()=>_q('xl-in').click());wrap.appendChild(drop);
  ...
  function _exportExcel(){
    const data=DB.exportAll();const wb=XLSX.utils.book_new();const clsRows=[];
    data.classes.forEach(cls=>{const mk=DB.monthKey(new Date());const bks=cls.monthBooks?.[mk]||{main:[],sub:[],pool:[]};clsRows.push({반:cls.name,요일:(cls.days||[]).join(','),등록일:cls.createdAt||'',교재목록:(bks.pool||[]).map(b=>b.name).join('/'),주교재:(bks.main||[]).map(b=>b.name).join('/'),부교재:(bks.sub||[]).map(b=>b.name).join('/')});});
    XLSX.utils.book_append_sheet(wb,XLSX.utils.json_to_sheet(clsRows),'반목록');
    const pRows=[];Object.entries(data.progress||{}).forEach(([k,v])=>{const p=k.split('__');const cls=data.classes.find(c=>c.id===p[0]);pRows.push({반:cls?.name||p[0],주차:p[1]||'',요일:p[2]||'',교재:p[3]||'',구분:p[4]||'',값:v});});
    XLSX.utils.book_append_sheet(wb,XLSX.utils.json_to_sheet(pRows),'진도데이터');
    XLSX.utils.book_append_sheet(wb,XLSX.utils.json_to_sheet([{data:JSON.stringify({version:9,classes:data.classes,progress:data.progress,theme:data.theme})}]),'_restore');
    const now=new Date();XLSX.writeFile(wb,`진도관리백업_${now.getFullYear()}${String(now.getMonth()+1).padStart(2,'0')}${String(now.getDate()).padStart(2,'0')}.xlsx`);
    _toast('📤 백업 완료','success');
  }

  function handleImport(input){const f=input.files[0];if(!f)return;input.value='';_processImport(f);}
  async function _processImport(file){const reader=new FileReader();reader.onload=async(e)=>{try{const wb=XLSX.read(e.target.result,{type:'array'});const raw=wb.Sheets['_restore'];if(!raw){_toast('⚠️ 올바른 백업 파일이 아닙니다','error');return;}const rows=XLSX.utils.sheet_to_json(raw);if(!rows[0]?.data){_toast('⚠️ 데이터 없음','error');return;}const data=JSON.parse(rows[0].data);const result=await DB.importAll(data);_renderMgCls();_renderChips();_renderDays();const div=document.createElement('div');div.className='import-result';div.innerHTML=`<div class="ir-added">✅ 추가: ${result.added.join(', ')||'없음'}</div><div class="ir-updated">🔄 업데이트: ${result.updated.join(', ')||'없음'}</div>`;_q('mg-io')?.appendChild(div);_toast('📥 복원 완료!','success');}catch(err){_toast('⚠️ 파일 오류: '+err.message,'error');console.error(err);}};reader.readAsArrayBuffer(file);}

  function _renderMgShare(){
    const wrap=_q('mg-share');if(!wrap)return;wrap.innerHTML='';
    const note=document.createElement('div');note.style.cssText='font-size:11px;color:var(--tx2);margin-bottom:10px;line-height:1.6';note.textContent='공유 링크 접속 시 해당 반만 읽기 전용으로 표시됩니다. Web Share API를 통해 카카오·이메일·문자 등으로 공유 가능합니다.';wrap.appendChild(note);
    const card=document.createElement('div');card.className='share-card';const classes=DB.getClasses();
    if(!classes.length){card.innerHTML='<div class="empty">등록된 반이 없습니다</div>';wrap.appendChild(card);return;}
    classes.forEach(cls=>{const row=document.createElement('div');row.className='share-cls-row';const url=`${location.origin}${location.pathname}?share=${cls.id}&wk=${DB.toWeekKey(S.monday)}`;row.innerHTML=`<div class="share-cls-name">${_esc(cls.name)}</div><div class="share-btns"><button class="share-btn copy" onclick="App.shareUrl('${_esc(url)}','${_esc(cls.name)}')">📤 공유</button><button class="share-btn sms" onclick="App.sendSms('${_esc(url)}','${_esc(cls.name)}')">💬 문자</button></div>`;card.appendChild(row);});wrap.appendChild(card);
  }
  ...
  function _renderShareView(classId,wkParam){
    ...
    if(!cls){view.innerHTML='<div class="empty" style="margin-top:80px;color:var(--tx3)">반 정보를 찾을 수 없습니다.<br><small>링크가 만료되었거나 반이 삭제되었습니다.</small></div>';return;}
    const wk=DB.toWeekKey(monday);const saved=DB.getWeekProgress(cls.id,wk);
    const fri=_addDays(monday,4);const fmt=d=>`${d.getMonth()+1}/${d.getDate()}`;
    view.innerHTML=`<div style="padding:13px 15px;background:var(--surf);border-bottom:1px solid var(--bdr);position:sticky;top:0;z-index:10;box-shadow:var(--sh)"><div style="font-size:18px;font-weight:800;color:var(--tx)">📚 ${_esc(cls.name)}반 진도 현황</div><div style="font-size:13px;color:var(--tx2);margin-top:2px;font-weight:600">${fmt(monday)} – ${fmt(fri)} · ${_wom(monday)}주차</div><div class="sv-ro-badge">🔒 읽기 전용</div></div><div id="sv-body" style="padding:11px;display:flex;flex-direction:column;gap:9px;background:var(--bg)"></div>`;
    const body=_q('sv-body');
    DAYS.forEach((dayName,i)=>{
      if(!(cls.days||[]).includes(dayName))return;
      const date=_addDays(monday,i);const mk=DB.monthKey(date);const books=DB.getMonthBooks(cls.id,mk);const dc=DC[dayName];
      const mainBooks=books.main||[];const subBooks=books.sub||[];
      const card=document.createElement('div');card.style.cssText='background:var(--card);border:1px solid var(--bdr);border-radius:14px;overflow:hidden;box-shadow:var(--sh)';
      card.innerHTML=`<div style="display:flex;align-items:center;gap:8px;padding:10px 12px;border-bottom:1px solid var(--bdr);background:var(--surf2)"><div class="bg-${dc}" style="width:4px;height:28px;border-radius:3px;flex-shrink:0"></div><div><div class="col-${dc}" style="font-size:15px;font-weight:700">${dayName}요일</div><div style="font-size:11px;color:var(--tx2)">${date.getMonth()+1}월 ${date.getDate()}일</div></div></div>`;
      if(mainBooks.length||subBooks.length){
        ...
        if(mainBooks.length){const sl=document.createElement('div');sl.style.cssText='font-size:10px;font-weight:800;color:var(--tx3);letter-spacing:1px;padding:3px 2px';sl.textContent='📘 주교재';rows.appendChild(sl);mainBooks.forEach(b=>rows.appendChild(_mkSvRow(b,'main',saved,dayName)));}
        if(subBooks.length){const sl=document.createElement('div');sl.style.cssText='font-size:10px;font-weight:800;color:var(--tx3);letter-spacing:1px;padding:5px 2px 3px';sl.textContent='📗 부교재';rows.appendChild(sl);subBooks.forEach(b=>rows.appendChild(_mkSvRow(b,'sub',saved,dayName)));}
    ...
    if(!S.shareActive){S.shareActive=true;DB.on('progress',()=>_renderShareView(classId,wkParam));}
  }

  function _mkSvRow(b,type,saved,dayName){
    const val=saved[`${dayName}__${b.id}__progress`]||'';const savedAt=saved[`${dayName}__${b.id}__savedAt`]||'';const dateStr=savedAt?new Date(savedAt).toLocaleDateString('ko-KR',{month:'2-digit',day:'2-digit'}):'';
    const brow=document.createElement('div');brow.style.cssText='display:flex;align-items:center;gap:7px;background:var(--card2);border:1px solid var(--bdr);border-radius:9px;padding:8px 10px';
    brow.innerHTML=`<span class="bk-tag ${type}">${type==='main'?'주':'부'}</span><span style="flex:1;font-size:12px;font-weight:600;color:var(--tx);overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${_esc(b.name)}</span><div style="text-align:right;flex-shrink:0"><div class="sv-bk-range ${val?'':'sv-bk-empty'}">${_esc(val)||'미입력'}</div>${dateStr?`<div style="font-size:9px;color:var(--tx3);margin-top:1px">${dateStr}</div>`:''}</div>`;
    return brow;
  }
  ...
  return {init,go,mgTab,setView,cancelLogin,doLogin,logout,prevWeek,nextWeek,openCal,closeCal,calPrev,calNext,calToday,openClassModal,saveClass,delClass,mgPrev,mgNext,openAccModal,saveAccount,delAcc,handleImport,shareUrl,sendSms,shareCurrentClass,closeModal};
})();
