 ...
 *
 * 신규/수정:
 * 1. 일반모드 read-only 입력값 가시성 (--readonly-val CSS)
 * 2. 타이틀 "수업 진도"로 변경
 * 3. 공통 월이동 바 (반 목록 상단 한 곳)
 * 4. Drag&Drop 500ms long-press 후에만 활성화
 * 5. 자동 로그아웃 30분
 * 6. 달력 팝업 컴팩트 + 날짜 옆 달력 아이콘
 * 7. 메모 min-height 3.2em, pre-wrap, auto-resize
 * 8. 주차 아래 월 표시 13px bold
 * 9. 오늘 요일 카드 하이라이트
 * 10. ID+PW 기억하기
 * 11. Android 하드웨어 백버튼 처리
 * 12. 단일 토글 버튼 (보기 현재 상태 표시)
 * 13. 로고 이미지 (logo.js) 배지
 * 14. 인트로 화면 해피트리 3D 애니메이션
 */
const App = (() => {
  ...
  const LS_REM='hk10_rem_id';
  const LS_REM_PW='hk10_rem_pw';
  ...
  function _resetAutoLogout(){
    ...
    _autoLogoutTimer=setTimeout(()=>{
      if(DB.isLoggedIn()){
        ...
        _toast('⏰ 30분 미사용으로 자동 로그아웃되었습니다');
      }
    },AUTO_LOGOUT_MS);
  }
  function _touchActivity(){_resetAutoLogout();}
  ...
  async function init(){
    ...
    // 활동 감지 → 자동 로그아웃 리셋
    ['touchstart','mousedown','keydown'].forEach(ev=>document.addEventListener(ev,_touchActivity,{passive:true}));
  ...
  function _setLogoImages(){
    ...
    const ids=['spl-logo-img','op-logo'];
    ids.forEach(id=>{const el=document.getElementById(id);if(el)el.src=LOGO.small;});
  }

  function _hideSplash(){
    const sp=document.getElementById('splash');sp.classList.add('out');
    setTimeout(()=>{sp.style.display='none';document.getElementById('app').classList.remove('hidden');go('operate');},480);
  }
  ...
  function _applyTheme(t){
    ...
    const ff=t.fontFamily||'Noto Sans KR';rs.setProperty('--font',`'${ff}',sans-serif`);document.body.style.fontFamily=`'${ff}',sans-serif`;
    const fz=t.fontSize||14;rs.setProperty('--fz',`${fz}px`);rs.setProperty('--fzs',`${Math.round(fz*.79)}px`);rs.setProperty('--fzm',`${Math.round(fz*1.14)}px`);rs.setProperty('--fzl',`${Math.round(fz*1.36)}px`);rs.setProperty('--fzh',`${Math.round(fz*1.64)}px`);
  ...
  function _onBack(e){
    ...
    // 모달 닫기 우선
    const modals=['login-gate','modal-cls','modal-acc','cal-ov'];
  ...
  function _showLogin(){
    ...
    const savedPw=localStorage.getItem(LS_REM_PW)||'';
    _q('li-id').value=savedId;_q('li-pw').value=savedPw;_q('li-err').textContent='';
    _q('li-remember').checked=!!(savedId);
    _q('login-gate').classList.remove('hidden');
    history.pushState({pg:'login'},'');
    setTimeout(()=>(savedPw?_q('li-pw'):savedId?_q('li-pw'):_q('li-id')).focus(),300);
  }
  ...
  function logout(){
    ...
    DB.clearSession();clearTimeout(_autoLogoutTimer);_refreshAuthUI();go('operate');_toast('로그아웃 되었습니다');
  }
  ...
  function _updateToggleBtn(){
    const btn=_q('toggle-view-btn');
    if(!btn)return;
    // 현재 viewMode 표시: grid=⊞, list=☰
    btn.textContent=S.viewMode==='grid'?'⊞':'☰';
    btn.title=S.viewMode==='grid'?'그리드 보기 (클릭시 리스트)':'리스트 보기 (클릭시 그리드)';
  }

  function toggleView(){
    S.viewMode=S.viewMode==='grid'?'list':'grid';
    _updateToggleBtn();
    const t=DB.getTheme();t.viewMode=S.viewMode;DB.saveTheme(t);
    _renderMgCls();
  }
  ...
  function _renderMgCls(){
    ...
    bar.innerHTML=`<button onclick="App.mgPrev()" title="이전 달">‹</button><span class="mg-month-lbl">📅 ${mkY}년 ${mkM}월 교재</span><button onclick="App.mgNext()" title="다음 달">›</button>`;
  ...
  function _buildClsCard(cls,isAdmin){
    ...
    // 반 카드에는 월이동 바 없음 (공통 바 사용)
    card.innerHTML=`<div class="cls-chdr"><div class="cls-chdr-l"><div class="cls-nm">${_esc(cls.name)}</div><div class="dbadges">${dayBadges}</div></div><div class="cls-chdr-r">${isAdmin?`<button class="ibtn" onclick="App.openClassModal('${cls.id}')" title="수정">✏️</button><button class="ibtn red" onclick="App.delClass('${cls.id}')" title="삭제">🗑</button>`:''}</div></div>`;
  ...
  function _buildPoolItem(b,clsId,mk,isAdmin){
    const item=document.createElement('div');item.className='bm-pool-item';item.dataset.bookid=b.id;item.dataset.name=b.name;item.dataset.clsid=clsId;
    ...
    if(isAdmin){
      ...
      // ★ 500ms long-press 후 drag 활성화 (스크롤 중 실수 방지)
      _setupLongPressDrag(item,b.id,b.name,'pool',clsId,mk);
    }
  ...
  function _buildAssignZones(clsId,mk,books,isAdmin){
    ...
    ['main','sub'].forEach(zone=>{
      ...
      (books[zone]||[]).forEach(b=>{
        const item=document.createElement('div');item.className='bm-zone-item';item.dataset.bookid=b.id;item.dataset.name=b.name;
        ...
        if(isAdmin){
          ...
          const back=document.createElement('button');back.className='bm-back-btn';back.title='목록으로';back.textContent='↩';back.onclick=async(e)=>{e.stopPropagation();await DB.moveBook(clsId,mk,b.id,'pool');_toast('↩ 목록으로 이동');};item.appendChild(back);
          _setupLongPressDrag(item,b.id,b.name,zone,clsId,mk);
        }
  ...
  /* ★ 500ms long-press 후 drag 활성화 */
  let _drag={item:null,bookId:null,name:'',fromZone:null,clsId:null,mk:null,ghost:null,active:false};
  let _lpTimer=null;

  function _setupLongPressDrag(el,bookId,name,fromZone,clsId,mk){
    // HTML5 drag
    el.draggable=false; // 기본 비활성
    el.addEventListener('dragstart',e=>{if(!_drag.active){e.preventDefault();return;}_drag={..._drag,item:el,bookId,name,fromZone,clsId,mk};el.classList.add('dragging');e.dataTransfer.effectAllowed='move';e.dataTransfer.setData('text/plain',bookId);});
    el.addEventListener('dragend',()=>{el.classList.remove('dragging');el.draggable=false;_drag.active=false;document.querySelectorAll('.drop-hover').forEach(z=>z.classList.remove('drop-hover'));});

    // Touch long-press
    el.addEventListener('touchstart',e=>{
      _lpTimer=setTimeout(()=>{
        _drag={item:el,bookId,name,fromZone,clsId,mk,active:true};
        el.classList.add('dragging');
        const ghost=_q('drag-ghost');ghost.textContent=name;ghost.classList.remove('hidden');
        const t=e.touches[0];ghost.style.left=t.clientX+'px';ghost.style.top=t.clientY+'px';_drag.ghost=ghost;
        // 짧은 진동 피드백
        navigator.vibrate?.(30);
      },500);
    },{passive:true});
    el.addEventListener('touchmove',e=>{
      if(!_drag.active){clearTimeout(_lpTimer);return;}
      const t=e.touches[0];
    ...
    el.addEventListener('touchend',async e=>{
      ...
      el.classList.remove('dragging');_drag.active=false;
      document.querySelectorAll('.drop-hover').forEach(z=>z.classList.remove('drop-hover'));
      const under=document.elementFromPoint(t.clientX,t.clientY);
      const zoneEl=under?.closest('.bm-zone-list,.bm-pool-list');
      if(zoneEl){const toZone=zoneEl.dataset.zone;if(toZone&&toZone!==fromZone){await DB.moveBook(clsId,mk,bookId,toZone);_toast(`${toZone==='main'?'📘 주교재':toZone==='sub'?'📗 부교재':'📚 목록'}으로 이동`,'success');}}
    });
    el.addEventListener('touchcancel',()=>{clearTimeout(_lpTimer);_drag.active=false;if(_drag.ghost)_drag.ghost.classList.add('hidden');el.classList.remove('dragging');});
  }
  ...
  function openClassModal(id=null){S.editClsId=id;const cls=id?DB.getClassById(id):null;_q('mcls-t').textContent=id?'반 수정':'반 추가';_q('f-cname').value=cls?.name||'';document.querySelectorAll('#modal-cls .day-ck input').forEach(cb=>{cb.checked=cls?(cls.days||[]).includes(cb.value):false;});_q('modal-cls').classList.remove('hidden');history.pushState({pg:'modal'},'');}
  ...
  function _renderMgTheme(){
    ...
    FONTS.forEach(f=>{const item=document.createElement('div');item.className='ff-item'+(f.key===(t.fontFamily||'Noto Sans KR')?' on':'');item.style.fontFamily=`'${f.key}',sans-serif`;item.innerHTML=`<span class="ff-name">${f.label}</span><span class="ff-sample">${f.sample}</span>`;if(!isAdmin){item.style.pointerEvents='none';item.style.opacity='.45';}item.onclick=()=>{S.tmpTheme.fontFamily=f.key;_applyTheme(S.tmpTheme);ffList.querySelectorAll('.ff-item').forEach((el,i)=>el.classList.toggle('on',FONTS[i].key===f.key));};ffList.appendChild(item);});fr.appendChild(ffList);card.appendChild(fr);
    ...
    sl.addEventListener('input',()=>{S.tmpTheme.fontSize=+sl.value;fzv.textContent=`${sl.value}px`;_applyTheme(S.tmpTheme);});szW.appendChild(sl);szW.appendChild(fzv);szr.appendChild(szW);card.appendChild(szr);
    ...
    isl.addEventListener('input',()=>{const w=+isl.value;S.tmpTheme.inputBoxWidth=w;iwv.textContent=`${w}px`;_applyTheme(S.tmpTheme);const pb=_q('inp-prev-box');if(pb)pb.style.width=`${w}px`;});iwW.appendChild(isl);iwW.appendChild(iwv);iwr.appendChild(iwW);iwr.appendChild(ipWrap);card.appendChild(iwr);
  ...
  /* 공유 탭 */
  function _renderMgShare(){const wrap=_q('mg-share');if(!wrap)return;wrap.innerHTML='';const note=document.createElement('div');note.style.cssText='font-size:11px;color:var(--tx2);margin-bottom:10px;line-height:1.6';note.textContent='공유 링크 접속 시 해당 반만 읽기 전용으로 표시됩니다.';wrap.appendChild(note);const card=document.createElement('div');card.className='share-card';const classes=DB.getClasses();if(!classes.length){card.innerHTML='<div class="empty">등록된 반이 없습니다</div>';wrap.appendChild(card);return;}classes.forEach(cls=>{const row=document.createElement('div');row.className='share-cls-row';const url=`${location.origin}${location.pathname}?share=${cls.id}&wk=${DB.toWeekKey(S.monday)}`;row.innerHTML=`<div class="share-cls-name">${_esc(cls.name)}</div><div class="share-btns"><button class="share-btn copy" onclick="App.shareUrl('${_esc(url)}','${_esc(cls.name)}')">📤 공유</button><button class="share-btn sms" onclick="App.sendSms('${_esc(url)}','${_esc(cls.name)}')">💬 문자</button></div>`;card.appendChild(row);});wrap.appendChild(card);}
  ...
  function _renderShareView(classId,wkParam){
    ...
    DAYS.forEach((dayName,i)=>{
      ...
      const today=new Date();today.setHours(0,0,0,0);const isToday=date.toDateString()===today.toDateString();
  ...
  function _mkSvRow(b,type,saved,dayName){const val=saved[`${dayName}__${b.id}__progress`]||'';const savedAt=saved[`${dayName}__${b.id}__savedAt`]||'';const dateStr=savedAt?new Date(savedAt).toLocaleDateString('ko-KR',{month:'2-digit',day:'2-digit'}):'';const brow=document.createElement('div');brow.style.cssText='display:flex;align-items:center;gap:7px;background:var(--card2);border:1px solid var(--bdr);border-radius:9px;padding:8px 10px';brow.innerHTML=`<span class="bk-tag ${type}">${type==='main'?'주':'부'}</span><span style="flex:1;font-size:12px;font-weight:600;color:var(--tx);overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${_esc(b.name)}</span><div style="text-align:right;flex-shrink:0"><div class="sv-bk-range ${val?'':'sv-bk-empty'}">${_esc(val)||'미입력'}</div>${dateStr?`<div style="font-size:9px;color:var(--tx3);margin-top:1px">${dateStr}</div>`:''}</div>`;return brow;}
